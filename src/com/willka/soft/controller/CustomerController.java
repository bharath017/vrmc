		...
		try	
		{
			...
			if(action.equals("AddCustomer")) {
				...
				if(count>0) {
					...
					response.sendRedirect("CustomerAddressDetails.jsp");
				}else {
			...
			else if(action.equals("getCustomerListByGroupWise")) {
				response.setContentType("application/json");
				response.setCharacterEncoding("utf-8");
				Gson gson=new Gson();
				int business_id=(request.getParameter("business_id")==null || request.getParameter("business_id").trim().equals("undefined"))?
								0:Integer.parseInt(request.getParameter("business_id"));
				List<Map<String,Object>> list=dao.getCustomerListByBusinessGroup(business_id);
