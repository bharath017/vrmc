		...
		try {
			...
			if(action.equals("InsertPayment")) {
				...
				if(count>0) {
					...
					response.sendRedirect("CustomerPaymentList.jsp");
				}else {
				...
				data.put("data",dao.getPaymentList(from_date, to_date, payment_id, customer_id, business_id,plant_id, start, length));
				data.put("draw", draw);
				int recordCount=dao.countPaymentList(from_date, to_date, payment_id, customer_id, business_id, plant_id);
				data.put("recordsTotal", recordCount);
				data.put("recordsFiltered", recordCount);
				writer.print(gson.toJson(data));
			}
			
			else if(action.equals("getCustomerByMarketingPerson")) {
				response.setContentType("application/json");
				response.setCharacterEncoding("utf-8");
				Gson gson=new Gson();
				int mp_id=Integer.parseInt(request.getParameter("mp_id"));
				writer.println(gson.toJson(dao.getCustomerByMarketingPerson(mp_id)));
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
