...
import com.willka.soft.dao.ExpenseVoucherDAO;
import com.willka.soft.dao.ExpenseVoucherDAOImpl;
 ...
 */
@WebServlet("/ExpenseVoucherController")
public class ExpenseVoucherController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(action.equals("insertExpenseCategory")) {
				...
				synchronized (this) {
					...
					if(val>0){
						...
						response.sendRedirect("ExpenseVoucherList.jsp");
					}else{
			...
			else if(action.equals("addExpenseType")) {
				try {
					String expense_type=request.getParameter("expense_type");
					int count=dao.addExpenseType(expense_type);
					writer.println(count);
				}catch(Exception e) {
					writer.println(-1);
				}
			...
			else if(action.equals("updateExpenseType")) {
				try {
					String expense_type=request.getParameter("expense_type");
					String old_expense_type=request.getParameter("old_expense_type");
					int count=dao.updateExpenseType(expense_type, old_expense_type);
					writer.println(count);
				}catch(Exception e) {
					e.printStackTrace();
					writer.println(-1);
