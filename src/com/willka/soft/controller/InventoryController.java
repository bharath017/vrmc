		...
		try {
			...
			if(action.equals("InsertInventoryItem")) {
				...
				synchronized (request) {
					...
					boolean bill_exist=request.getParameter("bill_exist")==null?false:true;
					...
					if(count>0) {
						int inventory_id=dao.getMaxOfInvoiceId();
						if(bill_exist) {
							response.sendRedirect("AddPurchaseVoucher.jsp?inventory_id="+inventory_id+"&insertType=inventory");
						}else {
