...
import com.willka.soft.test.dao.PurchaseOrderDAO;
import com.willka.soft.test.dao.PurchaseOrderDAOImpl;
 ...
 */
@WebServlet("/PurchaseOrderControllerTest")
			...
			if (action.equals("insert")) {
				synchronized (this) {
					...
					if(count>0){
						...
						response.sendRedirect("gst/PurchaseOrderList.jsp");
				...
				ps=con.prepareStatement("select g1.poi_id,g1.rate,g2.product_id,g2.max_date,g1.product_name from( select t.product_id,max(t.po_date) as max_date from (select p.poi_id,p.rate,pro.product_name,po.po_date,p.product_id from test_purchase_order_item  p,test_purchase_order" + 
						...
						" from test_purchase_order_item p,test_purchase_order po,product pro where p.order_id=po.order_id" + 
				...
				ps=con.prepareStatement("select poi.*,p.product_name,g.gst_percent,g.gst_category as tax_group,po.rate_include_tax"
						+ " from test_purchase_order_item poi LEFT JOIN (test_purchase_order po,product p,gst_percent g)"
				...
				JSONArray array=new JSONArray();
				con=DBUtil.getConnection();
				ps=con.prepareStatement("select i.*,p.product_name from test_invoice i,test_purchase_order_item poi,test_purchase_order po,product p where i.poi_id=poi.poi_id and poi.product_id=p.product_id and poi.order_id=po.order_id and po.order_id=?");
				...
				con=DBUtil.getConnection();
				ps=con.prepareStatement("select po.order_type,poi.quantity as order_quantity, po.po_valid_till,(select sum(payment_amount) from test_customer_payment where customer_id=po.customer_id) as payment_amount,(select credit_limit from test_customer where customer_id=po.customer_id) as credit_limit, " + 
						"						 (select sum(quantity)  from test_invoice where poi_id=poi.poi_id) as invoice_sum, " + 
						"						 (SELECT sum(net_amount) FROM  test_invoice i1,test_purchase_order po1,test_purchase_order_item poi1  where i1.poi_id=poi1.poi_id and poi1.order_id=po1.order_id and  po1.order_id=po.order_id) as invoice_net, " + 
						"						 poi.poi_id " + 
						"						 from test_purchase_order_item poi LEFT JOIN test_purchase_order po ON poi.order_id=po.order_id where poi.poi_id=?");
				ps.setInt(1, poi_id);
				rs=ps.executeQuery();
