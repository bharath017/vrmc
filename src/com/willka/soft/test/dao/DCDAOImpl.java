...
public class DCDAOImpl implements DCDAO {
	...
	private static final String INSERT_INVOICE="call testdcsave(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String INSERT_INVOICE_MODIFICATION="insert into test_dc_modification(invoice_id,modification_type,comment,modification_user) values(?,?,?,?)";
	
	private static final String UPDATE_VEHICLE_PUMP="update test_dc set vehicle_no=?,driver_name=?,pump=? where id=?";
	
	private static final String UPDATE_DATE_TIME="update test_dc set invoice_date=?,invoice_time=? where id=?";
	
	private static final String UPDATE_FULL_INVOICE="update test_dc set plant_id=?, customer_id=?, site_id=?, poi_id=?, invoice_date=?, invoice_time=?, quantity=?, rate=?, gross_amount=?, tax_amount=?, net_amount=?, vehicle_no=?, pump=?, driver_name=?, project_block=?,km_reading=?,loaded_quantity=? where id=?";
	
	private static final String GENERATE_CONSOLIDATE_INVOICE="insert into test_consolidate_invoice(customer_id,pump_charge,advance_amount,plant_id,generate_date,cons_id,start_year,end_year) values(?,?,?,?,?,?,?,?)";
	...
	private static final String GET_MAX_CONS_ID="select max(cons_id) from test_consolidate_invoice where start_year=? and end_year=?";
	
	private static final String CANCEL_CONSOLIDATE_INVOICE="delete from test_consolidate_invoice where consolidate_invoice_id=?";
	
	private static final String REFRESH_DC="delete from test_dcbatchingsheet where id=?";
	
	private static final String CANCEL_INVOICE="update test_dc set invoice_status='cancelled' where id=?";
	
	private static final String CHANGE_DOCKET_NO="update test_dc set docket_no=? where id=?";
	
	private static final String GET_INVOICE_NO="select max(invoice_id) as max_invoice from test_dc where start_year=? and end_year=? and plant_id=?";
	
	private static final String DELETE_INVOICE="delete from test_dc where id=?";
	
	private static final String GET_MAX_OF_ID="select max(id) from test_dc";
	...
	private static final String DATE_WISE_REPORT=" select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,"
												...
												+ " c.customer_gstin from test_dc i LEFT JOIN (test_customer c,test_site_detail s,"
												+ " test_purchase_order_item poi,product pr,plant p,test_purchase_order po,"
	...
	private static final String CUSTOMER_WISE_REPORT=" select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,"
												...
												+ " c.customer_address,c.customer_gstin from test_dc i LEFT JOIN (test_customer c,test_site_detail s,test_purchase_order_item poi,"
												+ " product pr,plant p,test_purchase_order po,marketing_person m) "
	...
	private static final String MARKETING_PERSON_WISE_REPORT=" select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,"
												+ " c.customer_name,s.site_name,s.site_address,pr.product_name,"
												+ " pr.product_id,p.plant_name,m.mp_name,c.customer_address,"
												+ " c.customer_gstin from test_dc i LEFT JOIN (test_customer c,test_site_detail s,test_purchase_order_item poi,"
												+ " product pr,plant p,test_purchase_order po,marketing_person m) "
												+ " ON i.customer_id=c.customer_id "
												+ " and i.site_id=s.site_id "
												+ " and i.poi_id=poi.poi_id "
