...
public class CustomerDaoImpl implements CustomerDAO{
	...
	private static final  String INSERT_QUERY="INSERT INTO test_customer (customer_name, customer_phone,"
	...
	private static final String UPDATE_CUSTOMER_DETAILS="update test_customer set customer_name=?,customer_phone=?,customer_email=?,"
	...
	private static final String INSERT_SITE_DETAILS="insert into test_site_detail(site_name,site_address,customer_id) values(?,?,?)";
	
	private static final String UPDATE_SITE_DETAILS="update test_site_detail set site_name=?,site_address=?,tally_ladger=? where site_id=?";
	
	private static final String UPDATE_CUSTOMER_STATUS="update test_customer set customer_status=? where customer_id=?";
	
	private static final String GET_MAX_ID="select max(customer_id) as customer_id from  test_customer";
	
	private static final String SELECT_SITE_USING_CUSTOMER="select * from test_site_detail where customer_id=?";
	
	private static final String GET_SITE_ADDRESS="select * from test_site_detail where site_id=?";
	
	private static final String CHANGE_SITE_STATUS="update test_site_detail set site_status=? where site_id=?";
	
	private static final String GET_SINGLE_CUSTOMER_DETAILS="select * from test_customer where customer_id=?";
	
	private static final String UPDATE_TALLY_LADGER="update test_site_detail set tally_ladger=? where site_id=?";
	
	private static final String INSERT_CONTACT_PERSON="insert into test_customer_contact_person(contact_name,phone,email,customer_id) values(?,?,?,?)";
	
	private static final String UPDATE_CONTACT_PERSON="update test_customer_contact_person set contact_name=?,phone=?,email=? where contact_id=?";
	
	private static final String DELETE_CONTACT_PERSON="delete from test_customer_contact_person where contact_id=?";
	
	private static final String INSERT_PROJECT_BLOCK="insert into test_customer_site_block(block_name,site_id) values(?,?)";
	
	private static final String DELETE_PROJECT_BLOCK="delete from test_customer_site_block where block_id=?";
	
	private static final String GET_ALL_PROJECT_BLOCK="select * from test_customer_site_block where site_id=?";

	private static final String UPDATE_CREDIT_LIMIT = "update test_customer set credit_limit=? where customer_id=?";
	
	private static final String GET_ALL_CUSTOMER_LIST="select c.*,m.mp_name,b.group_name"
			+ " from test_customer c LEFT JOIN (marketing_person m,business_group b) "
			...
			+ " limit ?,?";
	private static final String COUNT_ALL_CUSTOMER_LIST="select count(customer_id) from test_customer "
			+ " where customer_name like ? "
			+ " and customer_phone like ? "
			+ " and customer_name like ?"
	...
	private static final String GET_MARKETING_PERSON_DETAILS_OF_CUSTOMER="select c.mp_id,m.mp_name"
			+ " from test_customer c,marketing_person m"
			+ " where c.mp_id=m.mp_id"
			+ " and  customer_id=?";
	
	public static final String GET_ALL_CUSTOMER_LIST_BY_BUSINESS_GROUP="select customer_id,customer_name"
			+ " from test_customer"
	...
	public ArrayList<HashMap<String, Object>> getAllCustomerList(String name, String phone, String status, String alp,int business_id,int plant_id,
		...
		while(rs.next()) {
			...
			map.put("last_dispatch_date", rs.getString("last_dispatch_date"));
			list.add(map);
		}
		return list;
	}
	...
	@Override
	public List<Map<String, Object>> getCustomerListByGroup(int business_id) throws Exception {
		ps=con.prepareStatement(GET_ALL_CUSTOMER_LIST_BY_BUSINESS_GROUP);
