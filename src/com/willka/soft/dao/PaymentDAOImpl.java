...
public class PaymentDAOImpl implements PaymentDAO{
	...
	private static final String INSERT_PAYMENT="insert into customer_payment(customer_id,payment_amount,payment_mode,check_dd_no, "
			+ "check_dd_validity,payment_date,payment_time,site_id,user_name,neft_no,bank_detail_id) values(?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_PAYMENT="update customer_payment set customer_id=?,payment_amount=?,payment_mode=?,"
			+ "check_dd_no=?,check_dd_validity=?,payment_date=?,site_id=?,neft_no=?,bank_detail_id=? where payment_id=?";
	
	private static final String GET_CREDIT_AMOUNT="select (select sum(net_amount) from invoice  "
			+ " where customer_id=? and invoice_status='active') as invoice_sum,(select sum(payment_amount)"
			+ " from customer_payment where customer_id=?) as payment_sum";
	
	private static final String GET_SINGLE_PAYMENT_DETAILS="select * from customer_payment where payment_id=?";
	
	private static final String INSERT_PAYMENT_MODIFICATION="insert into customer_payment_modification(payment_id,customer_id,payment_amount,modification_payment_amount,payment_mode,check_dd_no,check_dd_validity,payment_date,payment_time,modification_type,comment,modification_user) values(?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String DELETE_PAYMENT_DETAILS="delete from customer_payment where payment_id=?";
	...
	private static final String GET_PAYMENT_LIST="select p.*,c.customer_name,"
			...
			+ " from customer_payment p,customer c,bank_detail b"
	...
	private static final String GET_SALES_DOCUMENT_BALANCE_PAYMENT="select (select sum(sd.net_price) from sales_document s left join sales_document_item sd on sd.id=s.id where s.customer_id=? and s.invoice_status='active') as invoice_sum,"
			+ " (select sum(payment_amount) from customer_payment where customer_id=?) as payment_sum,"
			+ " (select sum(net_amount) from invoice where customer_id=?)";
	
	private static final String GET_CUSTOMER_BY_MP_ID="select customer_id,customer_name from customer where mp_id=? and  customer_status='active'";
	...
	public ArrayList<HashMap<String, Object>> getPaymentList(String from_date, String to_date, String payment_id,
			int customer_id, int business_id,int plant_id, int start, int length) throws Exception {
	...
	public List<Map<String, Object>> getCustomerByMarketingPerson(int mp_id) throws Exception {
		ps=con.prepareStatement(GET_CUSTOMER_BY_MP_ID);
		ps.setInt(1, mp_id);
		rs=ps.executeQuery();
