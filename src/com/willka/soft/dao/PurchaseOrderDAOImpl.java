...
public class PurchaseOrderDAOImpl implements PurchaseOrderDAO {
	...
	//declare query here
	private static final String INSERT_QUERY="insert into purchase_order(po_date,po_valid_till,tax_group,customer_id,site_id,"
			+ "marketing_person_id,plant_id,po_number,rate_include_tax,gst_percent,order_type,bill_photo,credit_days,credit_limit,gst_id,tcs_required"
			+ ") values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_QUERY="update purchase_order set po_date=?,po_valid_till=?,tax_group=?"
			+ ",customer_id=?,site_id=?,marketing_person_id=?,plant_id=?,po_number=?,"
			+ "rate_include_tax=?,gst_percent=?,order_type=?,bill_photo=?,credit_days=?,"
			+ "credit_limit=?,gst_id=?,tcs_required=? where order_id=?";
	
	private static final String INSERT_PURCHASE_ITEM="insert into purchase_order_item(product_id,quantity,rate,order_id) values(?,?,?,?)";
	
	private static final String UPDATE_PURCHASE_ITEM="update purchase_order_item set product_id=?,quantity=?,rate=?,order_id=? where poi_id=?";
	
	private static final String CANCEL_ORDER="update purchase_order set status='cancelled' where order_id=?";
	
	private static final String ACTIVATE_ORDER="update purchase_order set status='active' where order_id=?";
	
	private static final String GET_MAX_ORDER_ID="select max(order_id) from purchase_order";
	...
	private static final String GET_PURCHASE_ORDER_LIST="select po.order_id,c.customer_name,s.site_name,m.mp_name,DATE_FORMAT(po_date,'%d/%m/%Y') as po_date,"+
														...
														" from purchase_order po LEFT JOIN (customer c,site_detail s,gst_percent g,marketing_person m)"+
	...
	private static final String GET_PURCHASE_ORDER_ITEM_LIST="select poi.poi_id,poi.rate,poi.quantity,p.product_name"+
													" from purchase_order_item poi,product p"+
	...
	private static final String GET_SINGLE_PURCHASE_ORDER_DETAILS="select c.customer_name,s.site_name,m.mp_name,b.group_name,"+
			...
			" from purchase_order po LEFT JOIN (customer c,site_detail s,gst_percent g,marketing_person m,business_group b)"+
			" ON po.customer_id=c.customer_id"+
			" and po.site_id=s.site_id"+
			" and po.gst_id=g.gst_id"+
			" and po.marketing_person_id=m.mp_id"+
	...
	private static final String GET_SINGLE_PURCHASE_ORDER_DETAILS_ITEMS="select poi.poi_id,poi.rate,"+
			" poi.quantity,p.product_name,(select sum(quantity) from invoice where poi_id=poi.poi_id) as tquantity"+
			" from purchase_order_item poi,product p"+
			" where poi.product_id=p.product_id"+
			" and poi.order_id=?";
	
	private static final String GET_TCS_REQUIRED_POI_ID="select tcs_required from purchase_order  po,purchase_order_item poi "+
			" where po.order_id=poi.order_id "+
			" and poi.poi_id=?";
	
	private static final String GET_INVOICE_NET_AMOUNT_BY_FINANCIAL_YEAR_WISE="select sum(net_amount) from invoice where start_year=?"+
			" and end_year=? and customer_id=? and invoice_status='active'";
	...
	public int insertPurchaseOrder(PurchaseOrderBean bean) throws Exception {
		...
		ps.setBoolean(16, bean.isTcs_required());
	...
	@Override
	public boolean getTcsDetailsByPOIId(int poi_id) throws Exception {
		ps=con.prepareStatement(GET_TCS_REQUIRED_POI_ID);
		ps.setInt(1, poi_id);
		rs=ps.executeQuery();
		if(rs.next())
	...
	@Override
	public double getNetAmountBetweenFinancialYear(int start_year, int end_year, int customer_id) throws Exception {
		ps=con.prepareStatement(GET_INVOICE_NET_AMOUNT_BY_FINANCIAL_YEAR_WISE);
