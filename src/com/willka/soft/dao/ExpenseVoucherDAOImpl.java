...
import com.willka.soft.bean.ExpenseVoucherBean;
import com.willka.soft.bean.ExpenseVoucherItemBean;
...
public class ExpenseVoucherDAOImpl implements ExpenseVoucherDAO{
	...
	private static final String INSERT_EXPENSE_QUERY="insert into expense_voucher(bill_no,bill_date"
			+ ",tds_amount,plant_id,remark,supplier_id,category_type,gst_category,rate_include_tax)"
			+ " values(?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_QUERY="update expense_voucher set bill_no=?,bill_date=?,"
			+ " tds_amount=?,plant_id=?,remark=?,supplier_id=?,category_type=?,gst_category=?,rate_include_tax=?"
			+ " where expense_voucher_id=?";
	
	private static final String CANCEL_VOUCHER="update expense_voucher set  voucher_status='cancelled' where expense_voucher_id=?";
	
	private static final String INSERT_VOUCHER_ITEM="insert into expense_voucher_item(expense_voucher_id,"
			+ "item_name,item_quantity,item_price,gst_percent,gross_amount,tax_amount,net_amount,category_id) values(?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_VOUCHER_ITEM="update expense_voucher_item set  item_name=?,"
			+ "item_quantity=?,item_price=?,gst_percent=?,gross_amount=?,tax_amount=?,net_amount=?,category_id=? where evi_id=?";
	
	private static final String DELETE_VOUCHER_ITEM="delete from purchase_voucher_item where pvi_id=?";
	
	private static final String GET_MAX_ID="select max(expense_voucher_id) from expense_voucher";
	...
	private static final String GET_EXPENSE_VOUCHER_LIST="select p.expense_voucher_id,"
			...
			+ " p.rate_include_tax "
			+ " from expense_voucher p,supplier s where p.supplier_id=s.supplier_id"
			+ " and p.bill_no like ?"
	...
	private static final String GET_EXPENSE_VOUCHER_ITEM="select item_name,item_quantity,item_price,net_amount"
			+ " from expense_voucher_item"
			+ " where expense_voucher_id=?";
	
	private static final String DELETE_EXPENSE_VOUCHER="delete from expense_voucher where expense_voucher_id=?";
	
	private static final String GET_DASHBOARD="select (select sum(net_amount) from expense_voucher ex,expense_voucher_item exi "+
			...
			" (select sum(net_amount) from expense_voucher ex,expense_voucher_item exi "+
			" where ex.expense_voucher_id=exi.expense_voucher_id"+
			...
			" (select count(expense_voucher_id) from expense_voucher where bill_date=curdate() and voucher_status='active') as today";
	
	private static final String GET_EXPENSE_CATEGORY_TYPE="select category_type from expense_category_type";
	
	private static final String ADD_EXPENSE_TYPE="insert into expense_category_type(category_type) values(?)";
	
	private static final String UPDATE_EXPENSE_TYPE="update expense_category_type set category_type=? where category_type=?";
	...
	@Override
	public int addExpenseType(String expense_type) throws Exception {
		ps=con.prepareStatement(ADD_EXPENSE_TYPE);
		ps.setString(1, expense_type);
		int count=ps.executeUpdate();
		return count;
	}

	@Override
	public int updateExpenseType(String expense_type,String old_expense_type) throws Exception {
		ps=con.prepareStatement(UPDATE_EXPENSE_TYPE);
		ps.setString(1, expense_type);
		ps.setString(2, old_expense_type);
		int count=ps.executeUpdate();
		return count;
	}
