...
public class MakePaymentDAOImpl implements MakePaymentDAO {
	...
	private static final String GET_CREDIT_BALANCE="select (select sum(net_amount) from purchase_voucher p,purchase_voucher_item pi where p.purchase_voucher_id=pi.purchase_voucher_id and p.supplier_id=?) as total_purchase,(select sum(payment_amount) from make_payment where supplier_id=?) as total_payment";
	
	private static final String INSET_QUERY="insert into make_payment(payment_amount,payment_date,payment_time,payment_mode,check_dd_no,check_dd_validity,payment_details,supplier_id,remark,bank_detail_id) values(?,?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_QUERY="update  make_payment set  payment_amount=?,payment_date=?,payment_time=?,payment_mode=?,check_dd_no=?,check_dd_validity=?,payment_details=?,supplier_id=?,remark=?,bank_detail_id=? where payment_id=?";
	
	private static final String DELETE_QUERY="delete from make_payment where payment_id=?";
	
	private static final String GET_SINGLE_MAKE_PAYMENT="select * from make_payment where payment_id=?";
	
	private static final String INSERT_MAKE_PAYMENT_MODIFICATION="insert into make_payment_modification(payment_id,payment_date,"
	...
	private static final String GET_MAKE_PAYMENT_LIST="select m.*,s.supplier_name"
			...
			+ " from make_payment m,supplier s,bank_detail b"
	...
	private static final String GET_DATE_WISE_REPORT="select p.payment_id,p.payment_amount,DATE_FORMAT(p.payment_date,'%d/%m/%Y') as payment_date,"+
			...
			" from make_payment p LEFT JOIN (supplier s,bank_detail b)"+
	...
	private static final String GET_BANK_DATE_WISE_REPORT="select p.payment_id,p.payment_amount,DATE_FORMAT(p.payment_date,'%d/%m/%Y') as payment_date,"+
			" p.payment_mode,p.check_dd_no,p.check_dd_validity,s.supplier_id,s.supplier_name,b.bank_name,p.remark"+
			" from make_payment p LEFT JOIN (supplier s,bank_detail b)"+
			" ON p.supplier_id=s.supplier_id"+
			" and p.bank_detail_id=b.bank_detail_id"+
			" where s.business_id like if(?=0,'%%',?)"+
			" and p.payment_date between ? and ?"+
			" and p.bank_detail_id=?"+
			" order by s.supplier_name,s.supplier_id,payment_id asc";
	
	private static final String GET_TRANSACTION_PURCHASE_PAYMENT="select supplier_name,(select sum(net_amount) from purchase_voucher_item pvi,purchase_voucher pv"+
			 ...
			 " (select sum(payment_amount) from make_payment where supplier_id=s.supplier_id and payment_date between ? and ?) as totalPaid"+
			 " from supplier s"+
			 " where s.business_id like if(?=0,'%%',?)";
	...
	public List<Map<String, Object>> getBankWithDateWise(int bank_detail_id, String from_date, String to_date, int business_id) throws Exception {
		ps=con.prepareStatement(GET_BANK_DATE_WISE_REPORT);
		ps.setInt(1, business_id);
		ps.setInt(2, business_id);
		ps.setString(3, from_date);
		ps.setString(4, to_date);
