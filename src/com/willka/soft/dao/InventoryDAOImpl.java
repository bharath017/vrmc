...
import com.willka.soft.bean.Inventory;
import com.willka.soft.bean.InventoryItemBean;
import com.willka.soft.bean.InventoryItemModification;
import com.willka.soft.bean.InventoryModificationBean;
...
public class InventoryDAOImpl implements InventoryDAO{
	...
	private static final String GET_MAX_INVENTORY="select max(inventory_id) from inventory";
	...
	private static final String INSERT_INVENTORY="insert into inventory(inv_item_id,date,time,empty_weight,loaded_weight,net_weight,supplier_weight,"
			+ " bill_no,plant_id,supplier_id,vehicle_no,gatepass_no,unit,moisture_percent,after_weight,pl_delivery_address_id,royalty_no) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_INVENTORY="update inventory set inv_item_id=?,date=?,time=?,empty_weight=?, "
	...
	private static final String INSERT_INVENTORY_MODIFICATION="insert into inventory_modification(inventory_id,old_net_weight,new_net_weight,old_supplier_weight,new_supplier_weight,modification_user,modification_comment) values(?,?,?,?,?,?,?)";
	...
	private static final String CANCEL_INVENTORY="update inventory set inventory_status='deactive' where inventory_id=?";
	
	private static final String GET_SINGLE_INVENTORY="select * from inventory where inventory_id=?";
	...
	private static final String  GET_INVENTORY_REPORT="select i.inventory_id,it.inventory_name,DATE_FORMAT(i.date,'%d/%m/%Y') as date,"+
			...
			" from inventory i LEFT JOIN (supplier s,inventory_item it,plant p)"+
	...
	private static final String GET_SUPPLIER_WISE_REPORT="select i.supplier_id,s.supplier_name,"+
													...
													" from inventory i,supplier s,inventory_item it "+
													" where i.inv_item_id=it.inv_item_id"+
	...
	@Override
	public int getMaxOfInvoiceId() throws Exception {
		ps=con.prepareStatement(GET_MAX_INVENTORY);
		rs=ps.executeQuery();
		if(rs.next())
			return rs.getInt(1);
		else
