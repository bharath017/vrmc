...
import com.willka.soft.bean.PurchaseVoucherBean;
import com.willka.soft.bean.PurchaseVoucherItemBean;
...
public  class PurchaseVoucherDAOImpl implements PurchaseVoucherDAO {
	...
	//declare query here
	private static final String INSERT_PURCHASE_VOUCHER="insert into purchase_voucher(bill_no,purchase_date,"
			+ "discount_amount,supplier_id,rate_include_tax,round_off,gst_type,plant_id) values(?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_PURCHASE_VOUCHER="update purchase_voucher set bill_no=?,purchase_date=?,discount_amount=?,supplier_id=?,rate_include_tax=?,round_off=?,gst_type=?,plant_id=? where purchase_voucher_id=?";
	
	private static final String CANCEL_PURCHASE_VOUCHER="update purchase_voucher set voucher_status='cancelled' where purchase_voucher_id=?";
	
	private static final String ACTIVATE_VOUCHER1="update purchase_voucher set voucher_status='active' where purchase_voucher_id=?";
	
	private static final String INSERT_PURCHAES_VOUCHER_ITEM="insert into purchase_voucher_item(purchase_voucher_id,item_name,item_quantity,item_price,gst_percent,gross_amount,tax_amount,net_amount) values(?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_PURCHASE_VOUCHER_ITEM="update purchase_voucher_item set item_name=?,item_quantity=?,item_price=?,gst_percent=?,gross_amount=?,tax_amount=?,net_amount=? where pvi_id=?";
	
	private static final String DELETE_PURCHASE_VOUCHER_ITEM="delete from purchase_voucher_item set pvi_id=?";
	
	private static final String GET_MAX_VOUCHER_ID="select max(purchase_voucher_id) from purchase_voucher";
	...
	private static final String GET_PURCHASE_VOUCHER_LIST="select p.purchase_voucher_id,"
			...
			+ " from purchase_voucher p,supplier s where p.supplier_id=s.supplier_id"
			+ " and p.bill_no like ?"
	...
	private static final String GET_PURCHASE_VOUCHER_ITEM="select item_name,item_quantity,item_price,net_amount"
			+ " from purchase_voucher_item"
			+ " where purchase_voucher_id=?";
	private static final String DELETE_VOUCHER = "delete from purchase_voucher where purchase_voucher_id=?";
