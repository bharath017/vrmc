...
public class SalesComissionDAOImpl implements SalesComissionDAO {
	...
	private static final String INSERT_PAYMENT="insert into sales_comission(customer_id,payment_amount,payment_mode,check_dd_no,check_dd_validity,payment_date,payment_time,paid_to,site_id) values(?,?,?,?,?,?,?,?,?)";
	
	private static final String UPDATE_PAYMENT="update sales_comission set customer_id=?,payment_amount=?,payment_mode=?,check_dd_no=?,check_dd_validity=?,payment_date=?,payment_time=?,paid_to=?,site_id=? where payment_id=?";
	
	private static final String GET_CREDIT_AMOUNT="select (select sum(net_amount) from invoice  where customer_id=? and invoice_status='active') as invoice_sum,(select sum(payment_amount) from customer_payment where customer_id=?) as payment_sum,(select sum(payment_amount) from sales_comission where customer_id=?) as comission";
	
	private static final String GET_SINGLE_PAYMENT_DETAILS="select * from sales_comission where payment_id=?";
	
	private static final String INSERT_PAYMENT_MODIFICATION="insert into sales_comission_modification(payment_id,customer_id,payment_amount,modification_payment_amount,payment_mode,check_dd_no,check_dd_validity,payment_date,payment_time,modification_type,comment,modification_user) values(?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static final String DELETE_PAYMENT_DETAILS="delete from sales_comission where payment_id=?";
	...
	public double getCreditAmmount(int customer_id) throws Exception {
		...
		if(rs.next()) {
			...
			comission=rs.getDouble(2);
		}
		return invoice_sum-payment_sum-comission;
	}
	@Override
