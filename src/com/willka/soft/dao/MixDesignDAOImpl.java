...
public class MixDesignDAOImpl implements MixDesignDAO {
	...
	private static final String INSERT_RECEIPE="INSERT INTO receipe (customer_id,site_id,plant_id,product_id,recipe_code,"
			...
			+ " aggregate6_name, aggregate6_value, aggregate7_name, aggregate7_value, aggregate8_name, aggregate8_value,"
			+ " aggregate9_name, aggregate9_value, aggregate10_name, aggregate10_value, aggregate11_name,aggregate11_value, cement_name)"
			+ " VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?)";
	
	private static final String UPDATE_RECEIPE="update receipe set customer_id=?,site_id=?,plant_id=?,product_id=?,recipe_code=?,"
			+ " aggregate1_name=?, aggregate1_value=?, aggregate2_name=?, aggregate2_value=?, aggregate3_name=?, aggregate3_value=?, "
			+ "aggregate4_name=?, aggregate4_value=?, aggregate5_name=?, aggregate5_value=?, aggregate6_name=?,"
			+ " aggregate6_value=?, aggregate7_name=?, aggregate7_value=?, aggregate8_name=?, aggregate8_value=?,"
			+ " aggregate9_name=?, aggregate9_value=?, aggregate10_name=?, aggregate10_value=?, aggregate11_name=?, aggregate11_value=?, cement_name=? where receipe_id=?";
	
	private static final String DELETE_RECEIPE="delete from receipe where receipe_id=?";
	...
	private static final String GET_SINGLE_RECEIPE="select r.*,c.customer_name,s.site_name,p.product_name from receipe r LEFT JOIN (customer c,site_detail s,product p) ON r.customer_id=c.customer_id and r.site_id=s.site_id and r.product_id=p.product_id where receipe_id=?";
