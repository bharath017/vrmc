...
<sql:query var="mix" dataSource="jdbc/rmc">
	select distinct NO_OF_CYCLES_VAL0,DATE_FORMAT(date(timestamp),'%d/%m/%Y') as real_date,TRUCK_DETAIL_VAL0,POURING_DET_VAL0,MIX_DESIGN_VAL0,RECIPE_NO_VAL0,round(BATCH_SIZE_VAL0,2) as batch_size,CUST_DETAIL_VAL0,TOTAL_M3_VAL0 from indus1  where NO_OF_CYCLES_VAL0=? limit 1
  ...
  <sql:query var="time" dataSource="jdbc/rmc">
  	select  time(min(timestamp)) as min_time,time(max(timestamp)) as max_time from  indus1 where NO_OF_CYCLES_VAL0=?
  	<sql:param value="${param.id}"/>
  </sql:query>
  ...
  <sql:query var="rcp" dataSource="jdbc/rmc">
  		select * from indus1 where NO_OF_CYCLES_VAL0=? limit 1
  		<sql:param value="${param.id}"/>
  </sql:query>
  <c:forEach items="${rcp.rows}" var="rcp">
   ...
   <sql:query var="gen" dataSource="jdbc/rmc">
 		select  distinct * from indus1 where NO_OF_CYCLES_VAL0=?
 	<sql:param value="${param.id}"/>
 </sql:query>
...
<!-- End Line -->
 ...
 <sql:query var="sum" dataSource="jdbc/rmc">
  		...
  		from (select DISTINCT * from indus1 where   NO_OF_CYCLES_VAL0=?) as t ;
  		<sql:param value="${param.id}"/>
 </sql:query>
 ...
 <sql:query var="dsum" dataSource="jdbc/rmc">
  	    ...
  	    from indus1 where   NO_OF_CYCLES_VAL0=? limit 1 ;
  		<sql:param value="${param.id}"/>
 </sql:query>
