    ...
    <body>
        ...
        <div class="wrapper">
            <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="btn-group pull-right">
                                <ol class="breadcrumb hide-phone p-0 m-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">DC Report</a></li>
                                </ol>
                            </div>
                            <h4 class="page-title">DC Report</h4>
                        </div>
                    </div>
                </div>
                 <!-- end row -->
                <div class="row">
                	<div class="col-md-12">
                		<h2 class="text-info text-center">Generate DC Report</h2><hr><br><br>
                	</div>
                    <div class="col-md-12">
                ...
                <form action="InvoiceController?action=generatecons" method="post" class="form" id="myfrm" style="margin: 0 !important;padding: 0 !important;">
               	...
               	<div class="row" style="margin-top: 25px;">
               		<div class="col-sm-12">
               			<a type="button" class="btn btn-warning btn-sm" id="downloadExcel">Excel</a>
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               						    ...
               						    from dc i LEFT JOIN (customer c,site_detail s,purchase_order_item poi,product pr,plant p,purchase_order po,marketing_person m)
			               				...
			               				<tr style="background-color: #4b9663;color: white;">
			               					...
			               					<c:catch var="ee">
			               						<sql:query var="customer" dataSource="jdbc/rmc">
				               						...
				               						from dc i LEFT JOIN(customer c,purchase_order_item poi)
				               						ON i.customer_id=c.customer_id 
				               						and i.poi_id=poi.poi_id
				               						where i.customer_id like if(0=?,'%%',?)
													and i.site_id like if(''=?,'%%',?)
				               				...
				               				<c:forEach var="customer" items="${customer.rows}">
				               					<c:catch var="er">
				               						<sql:query var="invv" dataSource="jdbc/rmc">
														...
														from dc i LEFT JOIN(customer c,site_detail s,product p,purchase_order_item poi,plant pl)
														ON i.customer_id=c.customer_id 
														and i.site_id=s.site_id 
														and i.poi_id=poi.poi_id 
               										...
               										<c:choose>
               											<c:when test="${invv.invoice_date==date}">
               												<tr>
								               					...
								               					<td><a href="PrintInvoice.jsp?id=${invv.id}" target="_blank">GST/${invv.invoice_id}</a></td>
               					...
               					<c:when test="${param.report_type=='vehicledate'}">
               						...
               						<table class="table table-bordered" id="example-2">
			               				...
			               				<tr style="background-color: #4b9663;color: white;">
			               					...
			               					<c:catch var="ee">
			               						<sql:query var="customer" dataSource="jdbc/rmc">
				               						select distinct c.vehicle_no 
				               						from dc i LEFT JOIN(vehicle c,purchase_order_item poi)
				               						ON i.vehicle_no=c.vehicle_no 
				               						and i.poi_id=poi.poi_id
				               				...
				               				<c:forEach var="customer" items="${customer.rows}">
				               					<c:catch var="er">
				               						<sql:query var="invv" dataSource="jdbc/rmc">
				               							select i.id,i.invoice_id,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') AS invoice_date,c.customer_name,s.site_address,
				               							i.quantity,i.net_amount,p.product_name,i.rate,i.gross_amount,pl.plant_name,i.vehicle_no
														from dc i LEFT JOIN(customer c,site_detail s,product p,purchase_order_item poi,plant pl,vehicle v)
														ON i.customer_id=c.customer_id 
														and i.site_id=s.site_id 
														and i.poi_id=poi.poi_id 
														and poi.product_id=p.product_id
														and i.plant_id=pl.plant_id
														and i.vehicle_no=v.vehicle_no
														where i.vehicle_no like if(0=?,'%%',?)
														and i.site_id like if(''=?,'%%',?)
														and poi.product_id like if(''=?,'%%',?)
														and i.plant_id like if(''=?,'%%',?)
														and i.invoice_date between ? and ?
														group by i.id,i.invoice_date,v.vehicle_no,c.customer_name,s.site_address,i.quantity,i.net_amount,
														p.product_name,i.rate,i.gross_amount,pl.plant_name,i.vehicle_no
														order by c.customer_name,s.site_address,i.invoice_date asc
				               							<sql:param value="${customer.vehicle_no}"/>
				               							<sql:param value="${customer.vehicle_no}"/>
				               							<sql:param value="${param.site_id}"/>
				               							<sql:param value="${param.site_id}"/>
				               							<sql:param value="${product_id}"/>
				               					...
				               					<tr>
				               						<td colspan="11"><h4>${customer.vehicle_no}</h4></td>
				               					</tr>
				               					<c:set value="" var="date"></c:set>
               									<c:set value="0" var="date_count"></c:set>
               									<c:set value="0" var="count"/>
									...
									<td class="d-none">Batch No.</td>
			               				...
			               				<c:forEach items="${inv.rows}" var="inv">
			               					...
			               					<tr>
				               					...
				               					<td><a href="PrintInvoice.jsp?id=${inv.id}" target="_blank">GST/${inv.invoice_id}</a></td>
										...
										<td class="d-none">${inv.batch_no}</td>
       ...
       <%@ include file="footer.jsp" %>
	    ...
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        }else if(report_type=='vehicle'){
	        	var vehicle_no=$('#vehicle_no').val();
	            var product_id=$('#product_id').val();
	            var plant_id=$('#plant_id').val();
	            window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	var plant_id=$('#plant_id').val();
	        	var block_name=$('#block_name').val();
	        	window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="DCReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&mp_id="+mp_id;
	        }
