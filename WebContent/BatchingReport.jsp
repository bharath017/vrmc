								...
								<c:choose>
									<c:when test="${empty param.action}">
										<sql:query var="batch" dataSource="jdbc/rmc">
											select sum(t.CEMTWT_VAL0)  as s5,sum(t.AGG1_VALUE_VAL0) as s1,
											sum(t.AGG2_VALUE_VAL0) as s2,sum(t.AGG3_VALUE_VAL0) as s3,sum(AGG4_VALUE_VAL0) as s4,
											sum(t.CEMTWT1_VAL0) as s6,sum(t.WTRWT_VAL0) as s7,sum(t.ADDWT_VAL0) as s8
											from (select distinct * from indus1 where date(timestamp)=curdate()) as t
										</sql:query>
									</c:when>
									<c:otherwise>
										<sql:query var="batch" dataSource="jdbc/rmc">
											...
											sum(t.AGG2_VALUE_VAL0) as s2,sum(t.AGG3_VALUE_VAL0) as s3,sum(AGG4_VALUE_VAL0) as s4,
											sum(t.CEMTWT1_VAL0) as s6,sum(t.WTRWT_VAL0) as s7,sum(t.ADDWT_VAL0) as s8
											from (select distinct * from indus1 where date(timestamp) between ? and ?) as t
