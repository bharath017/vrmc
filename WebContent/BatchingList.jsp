                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card-box table-responsive">
                             ...
                             <c:set value="${(empty param.batch_no)?0:param.batch_no}" var="batch_no"/>
                                <c:catch var="e">
                               		<sql:query var="batch" dataSource="jdbc/rmc">
                               			select BATCHID,TRIPNO,BATCHNO,CUSTOMER,RECIPE,SITE,TRUCK,DATE_FORMAT(BATCHDATE,'%d/%m/%Y %h:%m') as BATCHDATE,GRANDTOTAL
                               			from TRIPDETAIL
                               			where BATCHID LIKE if(?=0,'%%',?)
                               			and DATE(BATCHDATE) like if(?='','%%',?)
                            ...
                            <table id="example" class="table table-striped table-bordered table-condensed" cellspacing="0" width="100%">
                                ...
                                <c:forEach var="batch" items="${batch.rows}">
									<tr>
										...
										<td class="text-center">${batch.TRIPNO}</td>
										<td class="text-center">${batch.BATCHNO}</td>
										<td class="text-center">${batch.CUSTOMER}</td>
										<td class="text-center">${batch.RECIPE}</td>
										<td class="text-center">${batch.SITE}</td>
										<td class="text-center">${batch.TRUCK}</td>
										<td class="text-center">${batch.BATCHDATE}</td>
										<td class="text-center">${batch.GRANDTOTAL}</td>
