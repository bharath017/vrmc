				               				...
				               				<c:forEach var="customer" items="${customer.rows}">
				               					<c:catch var="er">
				               						<sql:query var="invv" dataSource="jdbc/rmc">
														...
														and i.plant_id like if(''=?,'%%',?)
														and i.invoice_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y')
														group by i.id,i.invoice_date,c.customer_name,s.site_address,si.item_quantity,si.net_price,
														p.product_name,si.item_price,si.gross_price,pl.plant_name,i.vehicle_no
