...
//set validation logic here
		...
		submitHandler: function(form) {
			...
			if(webcam){
				Webcam.snap(function(data_uri){
					Webcam.upload( data_uri, 'DCController?action=uploadPhoto', function(code, text) {
						$('#dcPhoto').val(text);
						if(submited==false){
						    form.submit();
						    submited=true;
	...
	$('#vehicle_no').on("change",function(){
		...
		getTicketId(vehicle_no);
		$.ajax({
			type:'POST',
			url:'VehicleController?button=GetEmptyWeight&vehicle_no='+vehicle_no,
			headers:{
				Accept:"application/json;charset=utf-8",
				"Content-Type":"application/json;charset=utf-8"
			},
			success:function(res){
				$('#empty_weight'). val(res.empty_weight);
	...
	var getTicketId=function(vehicle_no){
		$.ajax({
			type:'post',
			url:'TicketController?action=getLastTicketId&vehicle_no='+vehicle_no,
			success:function(res){
				$('#ticket_id').val(res);
			}
		});
