$(document).ready(function(){
   ...
   //view hide bill received or not 
    ...
    var table1 =   $('#example-2').DataTable({
        ...
        lengthChange: true,
        "ajax":{
        	url:"PettyCashController?action=GetPettyCashTransactionList",
        	type:"POST",
        	 data 	: function ( d ) {
	                  return $.extend( {}, d, {
        ...
        "columnDefs":[
        	...
        	{"targets":[1],"data":"date"},
        	{"targets":[2],"data":"amount"},
        	{"targets":[3],"data":"received_person"},
    ...
    $('#transaction-form').validate({
    	...
    	submitHandler:function(form){
    		...
    		if(transaction_id==null || transaction_id==0 || transaction_id==""){
    			...
    			$.ajax({
    				type:'GET',
    				url:'PettyCashController?action=InsertPettyCashTransaction',
    				data: $.param({
  ...
  //validate transaction update form here
    $('#transaction-update-form').validate({
    	...
    	submitHandler:function(form){
    			$.ajax({
    				type:'GET',
    				url:'PettyCashController?action=UpdatePettyCashTransaction',
    				data: $.param({
    ...
    $(document).on('click','#example-2 .edit-transacton-detail',function(){
    	...
    	$.ajax({
    		type:'post',
    		url:'PettyCashController?action=GetSinglePettyCashTransaction&transaction_id='+transaction_id,
    		headers:{
    			Accept:"application/json;charset=utf-8",
    			"Content-Type":"application/json;charset=utf-8"
    		},
  ...
  //Transaction bill received or not option
    $(document).on('click','#example-2 .is_bill_received',function(){
    	...
    	$.ajax({
    		type:'post',
    		url:'PettyCashController?action=changeTransactionBillStatus&transaction_id='+transaction_id+'&status='+status,
    		headers:{
    			Accept:'application/json;charset=utf-8',
    			"Content-Type":"application/json;charset=utf-8"
    		},
    ...
    $('#delete-btn').click(function(){
    	var transaction_id=$('#transaction_id-delete').val();
	    	$.ajax({
	    		type:'post',
	    		url:'PettyCashController?action=DeletePettyCashTransaction&transaction_id='+transaction_id,
	    		headers:{
	    			Accept:"application/json;charset=utf-8",
	    			"Content-Type":"application/json;charset=utf-8"
	    		},
	  ...
	  //make delete transaction work option
	    $('#approve-btn').click(function(){
	    	var cash_id=$('#cash_id-status').val();
		    	$.ajax({
		    		type:'post',
		    		url:'PettyCashController?action=ApprovePettyCash&cash_id='+cash_id,
		    		headers:{
		    			Accept:"application/json;charset=utf-8",
		    			"Content-Type":"application/json;charset=utf-8"
		    		},
    ...
    var getPettyCashDetails=function(){
    	$.ajax({
        	type:'post',
        	url:'PettyCashController?action=GetPettyCashDetails&plant_id='+session.plant_id,
        	headers:{
        		Accept:"application/json;charset=utf-8",
        		"Content-Type":"application/json;charset=utf-8"
        	},
