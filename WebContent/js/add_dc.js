$(document).ready(function(){
	...
	$('#quantity1').on('keyup blur',function(){
		calculateNetAmountforDCQuantity();
	});
	
	
	var calculateNetAmountforDCQuantity=function(){
		   var basic_rate=$('#rate').val();
	       basic_rate=(basic_rate=='')?0:parseFloat(basic_rate);
	       var quantity=$('#quantity').val();
	       quantity=(quantity=='')?0:parseFloat(quantity);
...
//set validation logic here
	...
	$('#site_id').on('change',function(){
		...
		$.ajax({
			type:'post',
			url:'PurchaseOrderController?action=getAllPoList&site_id='+site_id,
			headers:{
				Accept:"application/json;charset=utf-8",
				"Content-Type":"application/json;charset=utf-8"
			},
	...
	$('#order_id').on('change',function(){
		...
		$.ajax({
			type:'post',
			url:'PurchaseOrderController?action=getGradeListByOrderId&order_id='+order_id,
			headers:{
				Accept:"application/json;charset=utf-8",
				"Content-Type":"application/json;charset=utf-8"
			},
