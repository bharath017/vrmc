$(document).ready(function(){
	...
	var calculateNetAmount=function(){
	       ...
	       $('#grs_ammt').val(parseFloat(net_price).toFixed(2));
	       $('#net_price').val(parseFloat(net_price).toFixed(2));
		...
		var poi_id=$('#poi_id').val();
  		...
  		$.ajax({
  			...
  			success:function(res){
  				...
  				var present_cost=parseFloat((res.invoice_amount-res.payment_amount)+(rate*quantity));
					po_validity=res.po_validity;
  				...
  				if(res.order_type=='close'){
  					...
  					if(quantity > res.net_quantity){
  						...
  						$('#savebtn').prop("disabled",true);
  						$('#show-error').html("<td colspan='6' style='background-color:red;color:white;'>Credit Limit : "+res.credit_limit+" / Total Pending Amount : "+present_cost +" / Please Contact Incharge!!</td>");
  					}else{
					 ...
					 if (res.credit_limit!=0 && present_cost > res.credit_limit){
  	  						$('#savebtn').prop("disabled",true);
  	  						$('#show-error').html("<td colspan='6' style='background-color:red;color:white;'>Credit Limit : "+res.credit_limit+" / Total Pending Amount : "+present_cost +" / Please Contact Incharge!!</td>");
  	  					}else{
  						$('#savebtn').prop("disabled",false);
