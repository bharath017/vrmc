...
app.controller('myCtrl', function($scope, $http) {
	...
	$('.select2').css('width','100%').select2({allowClear:true})
		...
		var table =   $('#example').DataTable({
	        ...
	        "searching":true,
	        "ajax":{
	        	url:"SupplierController?action=getSupplierList",
	        	type:"POST",
	        	 data 	: function ( d ) {
		                  return $.extend( {}, d, {
	    ...
	    $(document).on('click','#example .view-supplier',function(){
			...
			$.confirm({
			    ...
			    onContentReady: function () {
			    	$.ajax({
			    	 type:'post',
			    	 url:'SupplierController?action=getSinglesupplierDetailsForUpdate&supplier_id='+supplier_id,
			    	 headers:{
			...
			submitHandler:function(form){
				if(submit==false){
					...
					if(supplier_id==0 || supplier_id=='' || supplier_id==null){
						...
						$http({
							  method: 'POST',
							  url: 'SupplierController?action=InsertSupplier&'+data
							})
	...
	$(document).on('click','#example .edit-supplier',function(){
    	...
    	$http({
    		method:'post',
    		url:'SupplierController?action=getSinglesupplierDetailsForUpdate',
    		headers:{
    			Accept:"application/json;charset=utf-8",
    			"Content-Type":"application/json;charset=utf-8"
    		},
