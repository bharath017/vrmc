$(document).ready(function(){
	...
	$('.picker').on('click',function(){
	 ...
	 var table =   $('#example').DataTable({
	        ...
	        lengthChange: true,
	        "ajax":{
	        	url:"SalesDocumentController?action=getSalesDocumentList",
	        	type:"POST",
	        	 data 	: function ( d ) {
		                  return $.extend( {}, d, {
	        ...
	        "columnDefs":[
	        	...
	        	"render":function(data,row){
	        		...
	        		if(data.invoice_status=='active'){
	        			...
	        			html+='<a class="dropdown-item view-po" target="_blank"  href="PrintSalesDocumentDC.jsp?id='+data.id+'"><i class="fa fa-print mr-2 text-muted font-18 vertical-middle"></i>Print DC</a>'
		            	...
		            	html+='<a class="dropdown-item" target="_blank" href="document/sales/'+data.documents+'"><i class="fa fa-eye mr-2 text-muted font-18 vertical-middle"></i>Download documents</a>'
	  ...
	  //proper proper option visibility
		    	    ...
		    	    buttons: {
		    	        confirm:{
		    	            ...
		    	            action: function(){
		    	                $.ajax({
		    	                	type:'post',
		    	                	url:'SalesDocumentController?action=CancelInvoice&id='+id,
	   ...
	   var getNotificationView=function(){
	    	$.ajax({
	    		type:'post',
	    		url:'PurchaseVoucherController?action=getNotificationDetails',
	    		headers:{
	    			Accept:"application/json;charset=utf-8",
	    			"Content-Type":"application/json;charset=utf-8"
	    		},
