$(document).ready(function(){
   var session=getSessionDetails();
	...
	var table =   $('#example').DataTable({
        ...
        lengthChange: true,
        "ajax":{
        	url:"CustomerController?action=GetAllCustomerList",
        	type:"POST",
        	 data 	: function ( d ) {
	                  return $.extend( {}, d, {
        ...
        "columnDefs":[
        	...
        	"render":function(data,row){
        		return '<b><a href="CustomerViewController?customer_id='+data.customer_id+'">'+data.customer_name+'</a></b>'
        	}},
        	...
        	"render":function(data,row){
        		if(data.customer_panno==undefined || data.customer_panno==null)
        			return 'N/A';
        		else return data.customer_panno;
        	}},
        	{"targets":[5],"data":"group_name"},
        	...
        	"render":function(data,row){
                ...
                html+='<a href="javascript: void(0);" class="table-action-btn dropdown-toggle arrow-none btn btn-light btn-sm" data-toggle="dropdown" aria-expanded="false"><i class="mdi mdi-dots-horizontal"></i></a>'
                	html+='<div class="dropdown-menu dropdown-menu-right">'
                		...
                		if(session.usertype=='superadmin' || session.usertype=='admin' || session.usertype=='account')
    ...
    $('#delete-btn').click(function(){
    	...
    	var customer_status=$('#customer_status-delete').val();
	    	$.ajax({
	    		type:'post',
	    		url:'CustomerController?action=changeCustomerStatus&customer_id='+customer_id+'&customer_status='+customer_status,
	    		headers:{
