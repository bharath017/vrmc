...
app.controller('myCtrl', function($scope, $http) {
	        ...
	        "columnDefs":[
	        	...
	        	"render":function(data,row){
	        		...
	        		if(data.user_status=='active')
	        			html+='<a id="'+data.user_id+'" name="'+data.user_name+'" status="inactive" style="cursor:pointer;" class="deactivate-user"><span class="text-danger"><i class="fa fa-window-close"></i></span></a>&nbsp&nbsp'
	        		else
	        			html+='<a id="'+data.user_id+'" name="'+data.user_name+'" status="active" style="cursor:pointer;" class="deactivate-user"><span class="text-danger"><i class="fa fa-check"></i></span></a>&nbsp&nbsp'
	                 return html;
	        	}}  	
	        ],
	...
	$(document).on('click','#example .edit-user',function(){
    	...
    	$http({
    		...
    		$('#plant_id').empty();
    		$('#plant_id').append('<option value="'+success.data.plant_id+'">'+success.data.user_plant_name+'</option>');
	...
	$(document).on('click','#example .deactivate-user',function(){
    	var user_id=$(this).attr('id');
    	var user_name=$(this).attr('name');
    	let user_status=$(this).attr('status');
    	$.confirm({
