$(document).ready(function(){
	...
	$('.picker').on('click',function(){
	 ...
	 var table =   $('#example').DataTable({
	        ...
	        lengthChange: true,
	        "ajax":{
	        	url:"PurchaseOrderController?action=getPurchaesOrderList",
	        	type:"POST",
	        	 data 	: function ( d ) {
		                  return $.extend( {}, d, {
	        ...
	        "columnDefs":[
	        	...
	        	"render":function(data,row){
	        		...
	        		html+=`<a class="dropdown-item" target="_blank" href="PrintPurchaseOrder.jsp?order_id=${data.order_id}"><i class="fa fa-print mr-2 text-muted font-18 vertical-middle"></i>Print PO</a>`
	    ...
	    $(document).on('click','#example .view-po',function(){
    			...
    			$.confirm({
    			    ...
    			    onContentReady: function () {
    			      ...
    			      $.ajax({
    			    	 type:'post',
    			    	 url:'PurchaseOrderController?action=getSinglePoDetails&order_id='+order_id,
    			    	 headers:{
    			    		 Accept:"application/json;charset=utf-8",
    			    		 "Content-Type":"application/json;charset=utf-8"
    			    	 },
		    	    ...
		    	    buttons: {
		    	        confirm:{
		    	            ...
		    	            action: function(){
		    	                $.ajax({
		    	                	type:'post',
		    	                	url:'PurchaseOrderController?action=cancelOrder&order_id='+order_id,
		    	                	success:function(res){
		    	                		table.draw();
		    	                		$.toast({
