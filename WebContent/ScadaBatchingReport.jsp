                          		...
                          		<div class="col-sm-12">
                                   <div class="form-group">
                                       ...
                                       <select class="select2" name="report_type" onchange="changeType()" id="report_type" required="required">
                                           ...
                                           <option value="customerdate" ${param.report_type=='customerdate'?'selected':''}>Customer With Date Wise(GROUP WISE)</option>
                                       </select>
                                   </div>
                               </div>
                               ...
                               <div class="col-sm-12 no-date cus no-veh cus-dat no-veh-dat no-mp no-mp-dat">
                                   <div class="form-group">
                                       ...
                                       <sql:query var="customer" dataSource="jdbc/rmc">
                                       	 select distinct(CUSTOMER) from TRIPDETAIL
                                       </sql:query>
                                       <select class="select2 no-date cus no-veh cus-dat no-veh-dat" name="customer_id" id="customer_id">
                                           ...
                                           <option value="${customer.CUSTOMER}" ${(customer.CUSTOMER==param.CUSTOMER)?'selected':''}>${customer.CUSTOMER  }</option>
                                           </c:forEach>
                                       </select>
                                   </div>
                               </div>
               			...
               			<c:choose>
               				<c:when test="${param.report_type=='date'}">
               					<table class="table" id="example-2">
		               				...
		               				<sql:query var="trip" dataSource="jdbc/rmc">
		               					select TRIPNO,BATCHNO,CUSTOMER,RECIPE,DATE_FORMAT(BATCHDATE,'%d/%m/%Y %h:%m') as BATCHDATE,
		               					ROUND(TOTALBIN1,2) as TOTALBIN1,ROUND(TOTALBIN2,2) as TOTALBIN2,ROUND(TOTALBIN3,2) AS TOTALBIN3,
		               					ROUND(TOTALBIN4,2) AS TOTALBIN4,ROUND(TOTALCEMENT1,2) AS TOTALCEMENT1,ROUND(TOTALCEMENT2,2) as TOTALCEMENT2,
		               					ROUND(TOTALADDTIVE1,2) as TOTALADDTIVE1,TOTALADDITIVE2,TOTALWATER,(select sum(FLYASH) from ABATCHDETAIL where TRIPNO=T.TRIPNO) as TOTALFLYASH
		               					FROM TRIPDETAIL T
		               					where DATE(BATCHDATE) between ? and ?
		               				...
		               				<c:set value="0" var="t10"/>
		               				<c:forEach items="${trip.rows}" var="trip">
		               					<tr>
			               					<td>${trip.TRIPNO}</td>
			               					<td>${trip.BATCHNO}</td>
			               					<td>${trip.BATCHDATE}</td>
			               					<td>${trip.CUSTOMER}</td>
			               					<td>${trip.RECIPE}</td>
			               					<td>${trip.TOTALBIN1}</td>
			               					<td>${trip.TOTALBIN2}</td>
			               					<td>${trip.TOTALBIN3}</td>
			               					<td>${trip.TOTALBIN4}</td>
			               					<td>${trip.TOTALCEMENT1}</td>
			               					<td>${trip.TOTALCEMENT2}</td>
			               					<td>${trip.TOTALFLYASH}</td>
			               					<td>${trip.TOTALWATER}</td>
			               					<td>${trip.TOTALADDTIVE1}</td>
			               					<td>${trip.TOTALADDTIVE2}</td>
			               					<c:set value="${t1+trip.TOTALBIN1}" var="t1"/>
				               				<c:set value="${t2+trip.TOTALBIN2}" var="t2"/>
				               				<c:set value="${t3+trip.TOTALBIN3}" var="t3"/>
				               				<c:set value="${t4+trip.TOTALBIN4}" var="t4"/>
				               				<c:set value="${t5+trip.TOTALCEMENT1}" var="t5"/>
				               				<c:set value="${t6+trip.TOTALCEMENT2}" var="t6"/>
				               				<c:set value="${t7+trip.TOTALFLYASH}" var="t7"/>
				               				<c:set value="${t8+trip.TOTALWATER}" var="t8"/>
				               				<c:set value="${t9+trip.TOTALADDTIVE1}" var="t9"/>
				               				<c:set value="${t10+trip.TOTALADDTIVE2}" var="t10"/>
				               			</tr>
		               				</c:forEach>
               				...
               				<c:when test="${param.report_type=='customerdate'}">
               					<sql:query var="customer" dataSource="jdbc/rmc">
									select distinct(CUSTOMER) from TRIPDETAIL where (CUSTOMER like if(''=?,'%%',?) or CUSTOMER IS NULL) and  date(BATCHDATE) between ? and ?
               					...
               					<table class="table" id="example-2">
		               				<tr style="background-color: #91e9eb;color: black;" class="text-center">
		               					<td colspan="15" class="text-center">
		               						<h3>Batch Report(Customer With Date(group))</h3>
		               					</td>
		               				</tr>
		               				<tr style="background-color: #9debb6;color: black;">
		               					<th>Batch No</th>
		               				...
		               				<c:forEach items="${customer.rows}" var="customer">
										<tr>
											<td colspan="8"><h5>${customer.CUSTOMER}</h5></td>
										</tr>
										<sql:query var="trip" dataSource="jdbc/rmc">
	               							select TRIPNO,BATCHNO,CUSTOMER,RECIPE,DATE_FORMAT(BATCHDATE,'%d/%m/%Y %h:%m') as BATCHDATE,
			               					ROUND(TOTALBIN1,2) as TOTALBIN1,ROUND(TOTALBIN2,2) as TOTALBIN2,ROUND(TOTALBIN3,2) AS TOTALBIN3,
			               					ROUND(TOTALBIN4,2) AS TOTALBIN4,ROUND(TOTALCEMENT1,2) AS TOTALCEMENT1,ROUND(TOTALCEMENT2,2) as TOTALCEMENT2,
			               					ROUND(TOTALADDTIVE1,2) as TOTALADDTIVE1,TOTALADDITIVE2,TOTALWATER,truncate(TOTALFLYASH,2) as TOTALFLYASH
			               					FROM TRIPDETAIL
			               					where CUSTOMER = ?
			               					and DATE(BATCHDATE) between ? and ?
			               					order by TRIPNO DESC
	               							<sql:param value="${customer.CUSTOMER}"/>
	               							<sql:param value="${param.from_date}"/>
	               							<sql:param value="${param.to_date}"/>
	               						</sql:query>
			               				...
			               				<c:set value="0" var="tt1"/>
			               				<c:set value="0" var="tt2"/>
			               				<c:set value="0" var="tt3"/>
			               				<c:set value="0" var="tt4"/>
			               				<c:set value="0" var="tt5"/>
			               				<c:set value="0" var="tt6"/>
			               				<c:set value="0" var="tt7"/>
			               				<c:set value="0" var="tt8"/>
			               				<c:set value="0" var="tt9"/>
			               				<c:set value="0" var="tt10"/>
	               						...
	               						<c:forEach items="${trip.rows}" var="trip">
			               					...
			               					<c:set value="${t1+tt1}" var="tt1"/>
				               				<c:set value="${t2+tt2}" var="tt2"/>
				               				<c:set value="${t3+tt3}" var="tt3"/>
				               				<c:set value="${t4+tt4}" var="tt4"/>
				               				<c:set value="${t5+tt5}" var="tt5"/>
				               				<c:set value="${t6+tt6}" var="tt6"/>
				               				<c:set value="${t7+tt7}" var="tt7"/>
				               				<c:set value="${t8+tt8}" var="tt8"/>
				               				<c:set value="${t9+tt9}" var="tt9"/>
				               				<c:set value="${t10+tt10}" var="tt10"/>
		               				</c:forEach>
	               						<tr style="background-color: #e3d39a;color: black;font-weight: bold;">
											<td colspan="5" class="text-right">Total: </td>
											<th>${tt1}</th>
       ...
       <%@ include file="footer.jsp" %>
	    ...
	    $("form").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="ScadaBatchingReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date;
	        }
	        	...
	        	var from_date=$('input[name="from_date"]').val();
	        	var to_date=$('input[name="to_date"]').val();
	        	window.location="ScadaBatchingReport.jsp?action=generateReport&report_type="+report_type+"&customer_id="+customer_id+"&from_date="+from_date+"&to_date="+to_date;
	        }
	    });
    </script>
