                 		...
                 		<c:choose>
                 				...
                 				<!-- Get customer details here -->
			               		...
			               		<div class="row">
			               				...
			               				<table class="table table-bordered table-hover table-condensed" id="example-2">
			               					...
			               					<tr>
			               						<td colspan="6" style="width:50%;">
			               							<table class="table table-bordered table-condensed">
			                							...
			                							<sql:query var="invoice" dataSource="jdbc/rmc">
			                								select t.* from (SELECT s.invoice_id as invoice_id,DATE_FORMAT(s.invoice_date,'%d/%m/%Y') as invoice_date,s.invoice_date as reeldate,c.customer_name,p.product_name,si.item_quantity as quantity,si.item_price as rate,si.net_price as net_amount,'Sales Document' as type,DATEDIFF(CURRENT_DATE(), s.invoice_date) as age,'' as plant_id FROM test_sales_document s  left join (`test_sales_document_item` si,test_customer c,product p,test_purchase_order_item poi) on s.id=si.id and c.customer_id=s.customer_id and p.product_id=poi.product_id and si.poi_id=poi.poi_id where c.customer_id=? and s.invoice_status='active'
			                								UNION
			                								SELECT '' as invoice_id,'' as invoice_date,'' as relldate,'' as customer_name,'' as product_name,'' as quantity,'' as rate,c.opening_balance as net_amount,'Opening Balance' as type,'' as age,'' as plant_id from test_customer c where customer_id=?
															UNION 
															select i.invoice_id,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,i.invoice_date as reeldate,c.customer_name,p.product_name,i.quantity,i.rate,i.net_amount,'Invoice' as type,DATEDIFF(CURRENT_DATE(), i.invoice_date) as age,i.plant_id from test_invoice i,test_purchase_order_item poi,product p,test_customer c where i.poi_id=poi.poi_id and poi.product_id=p.product_id and i.customer_id=c.customer_id and i.invoice_status='active' and i.customer_id=?
															UNION
															select db.id as invoice_id,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,i.invoice_date as reeldate,c.customer_name,p.product_name,db.quantity,db.approved_rate as rate,(-1*db.net_amount),'Credit Note' as type,DATEDIFF(CURRENT_DATE(), i.invoice_date) as age,'' as plant_id from debit_credit_note db,test_invoice i,test_purchase_order_item poi,product p,test_customer c where  db.invoice_id=i.id and i.poi_id=poi.poi_id and poi.product_id=p.product_id and i.customer_id=c.customer_id  and i.invoice_status='active' and i.customer_id=?
													) as t order by t.reeldate asc,t.invoice_id asc	
			                								<sql:param value="${param.customer_id}"/>
			                								<sql:param value="${param.customer_id}"/>
			                								<sql:param value="${param.customer_id}"/>
			                								<sql:param value="${param.customer_id}"/>
			               						...
			               						<sql:query var="payment" dataSource="jdbc/rmc">
			               							select * from test_customer_payment where customer_id=?
			               							<sql:param value="${param.customer_id}"/>
			               						</sql:query>
			               						...
			               						<sql:query var="sumnet" dataSource="jdbc/rmc">
			               							select sum(payment_amount) as paysum from test_customer_payment where customer_id=?
			               							<sql:param value="${param.customer_id}"/>
			               						</sql:query>
			               			...
			               			<div class="col-sm-12 text-center">
			               				...
			               				<a href="AddPayment.jsp" class="btn btn-info btn-sm" >Add New Payment</a>
			               			</div>
			               		</div>
                 			</c:when>
                 			...
                 			<fmt:formatDate var="todaydate" value="${today}" pattern="yyyy-MM-dd" />
                 			 <div class="input-group col-sm-4">
                                 <input type="text" name="invoice_date" class="form-control date-picker invoice_date" value="${(empty param.date)?(todaydate):(param.date)}"  placeholder="yyyy-mm-dd" id="id-date-picker-2" data-date-format="yyyy-mm-dd">
                                 <div class="input-group-append picker">
                                     <span class="input-group-text"><i class="mdi mdi-calendar"></i></span>
                                 </div>
                             ...
                             <button class="btn btn-success btn-sm hide"  id="generate_report" onclick="generateReport();">Generate Report</button>
                 				...
                 				<table class="table table-bordered table-hover table-dondensed printme" id="example-2">
                 					...
                 					<tr>
                 						<th colspan="13" class="text-center">Account Receivable statement  &nbsp;&nbsp; <b style="color:blue">${mydate}</b>  &nbsp;&nbsp;&nbsp;&nbsp;Business Group : <b style="color:blue">${(empty business_group)?'All Group':business_group}</b></th>
                 					</tr>
                 					...
                 					<sql:query var="ccustomer" dataSource="jdbc/rmc">
                 						select t.customer_id,t.customer_name,t.net,t.payment,t.credit_note,(IF(t.D120 IS NULL,0,t.D120)+IF(t.S120 IS NULL,0,t.S120)) as D120,
										...
										(select k.* from (select p.customer_id,p.customer_name,(if(p.net is null ,0,p.net)+if(p.snet is null,0,p.snet)+p.opening_balance) as net,if(p.payment is null,0,p.payment) as payment,p.credit_note,
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 120 and customer_id=p.customer_id and invoice_status='active') as 'D120',
										(select sum(net_price) from test_sales_document_item sd,test_sales_document s where sd.id=s.id and  datediff(?, s.invoice_date) > 120 and customer_id=p.customer_id and invoice_status='active') as 'S120',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 90 and datediff(?, invoice_date) <= 120 and customer_id=p.customer_id and invoice_status='active' ) as 'D91',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 90 and datediff(?, invoice_date) <= 120 and customer_id=p.customer_id and invoice_status='active' ) as 'S91',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 75 and datediff(?, invoice_date) <= 90 and customer_id=p.customer_id and invoice_status='active'  ) as 'D76',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 75 and datediff(?, invoice_date) <= 90 and customer_id=p.customer_id and invoice_status='active'  ) as 'S76',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 60 and datediff(?, invoice_date) <= 75 and customer_id=p.customer_id and invoice_status='active' ) as 'D61',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 60 and datediff(?, invoice_date) <= 75 and customer_id=p.customer_id and invoice_status='active' ) as 'S61',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 45 and datediff(?, invoice_date) <= 60 and customer_id=p.customer_id and invoice_status='active') as 'D46',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 45 and datediff(?, invoice_date) <= 60 and customer_id=p.customer_id and invoice_status='active') as 'S46',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 30 and datediff(?, invoice_date) <= 45 and customer_id=p.customer_id and invoice_status='active') as 'D31',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 30 and datediff(?, invoice_date) <= 45 and customer_id=p.customer_id and invoice_status='active') as 'S31',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 15 and datediff(?, invoice_date) <= 30 and customer_id=p.customer_id and invoice_status='active') as 'D16',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 15 and datediff(?, invoice_date) <= 30 and customer_id=p.customer_id and invoice_status='active') as 'S16',
										(select sum(net_amount) from test_invoice where datediff(?, invoice_date) > 0 and datediff(?, invoice_date) <= 15 and customer_id=p.customer_id and invoice_status='active') as 'D0',
										(select sum(net_price) from test_sales_document_item sdi,test_sales_document sd where sdi.id=sd.id and  datediff(?, invoice_date) > 0 and datediff(?, invoice_date) <= 15 and customer_id=p.customer_id and invoice_status='active') as 'S0',
										(select m.mp_name from marketing_person m,test_purchase_order po where po.marketing_person_id=m.mp_id and po.customer_id=p.customer_id limit 1) as mp_name
										from (select c.customer_id,c.customer_name,c.opening_balance,
										(select sum(net_amount) from test_invoice  where invoice_status='active' and invoice_date <=? and customer_id=c.customer_id and invoice_status='active')  as net,
										(select sum(sdi.net_price) from test_sales_document sd,test_sales_document_item sdi where sd.id=sdi.id and sd.invoice_status='active' and sd.invoice_date<=? and sd.customer_id=c.customer_id and sd.invoice_status='active') as snet,
										(select sum(payment_amount) from test_customer_payment where customer_id=c.customer_id and payment_date <= ?) as payment,
									...
									<c:set value="0" var="day120above"/>
                 					...
                 					<c:forEach items="${ccustomer.rows}" var="rs">
	                 					...
	                 					<!--<c:set value="0" var="D0payment"/>-->
	                 					<c:if test="${(rs.net-rs.payment-rs.credit_note)>0}">
                 						<tr>
			               					...
			               					<tr>
			               						<td colspan="6" style="width:50%;">
			               							<table class="table table-bordered table-condensed">
			                							...
			                							<!-- Get Invoice details here -->
			                							<sql:query var="invoice" dataSource="jdbc/rmc">
			                								SELECT '' as invoice_id,'' as invoice_date,'' as customer_name,'' as product_name,'' as quantity,'' as rate,c.opening_balance as net_amount,'Opening Balance' as type,'' as age,'' as plant_id from test_customer c where customer_id=?
															UNION 
			                								SELECT s.id as invoice_id,DATE_FORMAT(s.invoice_date,'%d/%m/%Y') as invoice_date,c.customer_name,p.product_name,si.item_quantity as quantity,si.item_price as rate,si.net_price as net_amount,'Sales Document' as type,DATEDIFF(CURRENT_DATE(), s.invoice_date) as age,'' as plant_id FROM test_sales_document s  left join (`test_sales_document_item` si,test_customer c,product p,test_purchase_order_item poi) on s.id=si.id and c.customer_id=s.customer_id and p.product_id=poi.product_id and si.poi_id=poi.poi_id where c.customer_id=? and s.invoice_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y') and s.invoice_status='active'
			                								UNION
															select i.invoice_id,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,c.customer_name,p.product_name,i.quantity,i.rate,i.net_amount,'Invoice' as type,DATEDIFF(CURRENT_DATE(), i.invoice_date) as age,i.plant_id from test_invoice i,test_purchase_order_item poi,product p,test_customer c where i.poi_id=poi.poi_id and poi.product_id=p.product_id and i.customer_id=c.customer_id and i.invoice_status='active' and i.customer_id=? and i.invoice_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y')
															UNION
			                							...
			                							<c:forEach items="${inv}" var="in">
				                							<tr>
				                								<td style="width:4%;" class="text-center">${in.idd}</td>
				                								<td style="width:3%;" class="text-center">${in.invoice_date}</td>
				                								<td style="width:4%;" class="text-center">${in.product_name}</td>
				                								<c:set value="${in.quantity+tsum}" var="tsum"/>
				                								<td style="width:5%;" class="text-center">${in.quantity}</td>
			               						...
			               						<!-- get payment details here -->
			               						<sql:query var="payment" dataSource="jdbc/rmc">
			               							select * from test_customer_payment where customer_id=? and payment_date between ? and ?
			               							<sql:param value="${param.customer_id}"/>
			               							<sql:param value="${param.from_date}"/>
			               							<sql:param value="${param.to_date}"/>
			               						</sql:query>
			               						...
			               						<!-- Now sum total payment according to perticular customer -->
			               						<sql:query var="sumnet" dataSource="jdbc/rmc">
			               							select sum(payment_amount) as paysum from test_customer_payment where customer_id=? and payment_date between ? and ?
			               							<sql:param value="${param.customer_id}"/>
			               							<sql:param value="${param.from_date}"/>
			               							<sql:param value="${param.to_date}"/>
			               						</sql:query>
    ...
    <script type="text/javascript">
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='cus'){
	        	...
	        	var customer_id=$('#customer_id').val();
	        	window.location="CreditReport.jsp?action=generateReport&business_id="+business_id+"&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&customer_id="+customer_id+"&plant_id="+plant_id;
	        }
	    });
    </script>
