                          		...
                          		<div class="col-sm-12">
                                   <div class="form-group">
                                       ...
                                       <select class="select2" name="report_type" id="report_type" required="required">
                                           ...
                                           <option value="datetime">Date &amp; Time Wise</option>
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							select pvi.*,pv.*,DATE_FORMAT(pv.purchase_date,'%d/%m/%Y') as realdate,s.supplier_name,DATE_FORMAT(pvi.timestamp,'%m/%d/%Y %H:%i') as realtimestamp
               							 from test_purchase_voucher_item pvi LEFT JOIN (test_purchase_voucher pv,supplier s)               							
               							...
               							order by pv.purchase_voucher_id asc
               					...
               					<c:when test="${param.report_type=='supplierdate'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							...
               							from test_purchase_voucher_item pvi LEFT JOIN (test_purchase_voucher pv)
               							ON pvi.purchase_voucher_id=pv.purchase_voucher_id
               							where pv.supplier_id like if(''=?,'%%',?)
    ...
    <script type="text/javascript">
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="PurchaseVoucherReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date;
	        }else if(report_type=='datetime'){
	        	var from_date=$('.from_date').val();
	        	var to_date=$('.to_date').val();
	        	var from_time=$('.from_time').val();
	        	var to_time=$('.to_time').val();
	        	window.location="PurchaseVoucherReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&from_time="+from_time+"&to_time="+to_time;
	        }else if(report_type=='supplier'){
	        	var supplier_id=$('#supplier_id').val();	        
	        	window.location="PurchaseVoucherReport.jsp?action=generateReport&report_type="+report_type+"&supplier_id="+supplier_id;
    ...
    <script type="text/javascript">
    	$(document).ready(function(){
    		$('#customer_id').on('change',function(){
    			...
    			$.ajax({
    				type:'POST',
    				url:'CustomerController?action=GetAllSite&customer_id='+customer_id,
    				headers:{
    				...
    				success:function(res){
    					$('#site_id').text('');
    					$('#site_id').append('<option value="" selected="selected">All Site</option>');
    					$.each(res,function(i,d){
    						$('#site_id').append('<option value="'+d.site_id+'">'+d.site_name+'</option>');
    					});
    				}
    			});
    		});
