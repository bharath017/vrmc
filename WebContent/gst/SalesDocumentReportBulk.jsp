               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							...
               							(select count(sditem_id) from test_sales_document_item where id=i.id) as count,
         ...
         <!-- Modal for tally ladger save  -->
				...
				<div id="tally_modal" class="modal-demo col-xs-2">
		            ...
		            <div class="custom-modal-text">
		                <form class="form-horizontal" action="../SalesDocumentControllerTest?action=UpdateTallyLedger" method="post">
    ...
    <script type="text/javascript">
    	$(document).ready(function(){
    		$('#tally').on("click",function(){
    			...
    			$.ajax({
    				type:'POST',
    				url:'../SalesDocumentControllerTest?action=TallyXml&report_type='+report_type+'&customer_id='+customer_id+'&from_date='+from_date+'&to_date='+to_date+'&site_id='+site_id+'&product_id='+product_id+'&plant_id='+plant_id+'&vehicle_no='+vehicle_no,
    				headers:{
    					Accept:"application/json;charset=utf-8",
    				    "Content-Type":"application/json;charset=utf-8"
    				},
