                 	...
                 	<c:when test="${param.report_type=='supday'}">
                 				...
                 				<table class="table table-bordered table-hover table-dondensed printMe" id="example-2">
                 					...
                 					<sql:query var="ccustomer" dataSource="jdbc/rmc">
                 						select t.supplier_id,t.supplier_name,(t.net+t.enet+t.opening_balance) as net,t.total_tds,t.payment,(IF(t.D120 IS NULL,0,t.D120)+t.opening_balance) as D120,
										...
										(select k.* from (select p.supplier_id,p.supplier_name,(select sum(pv.tds_amount) from test_purchase_voucher pv where pv.supplier_id=p.supplier_id  and pv.voucher_status='active') as total_tds,if(p.payment is null,0,p.payment) as payment,if(p.net is null,0,p.net) as net,if(p.enet is null,0,p.enet) as enet,p.opening_balance,
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 120 and pv.voucher_status='active') as 'D120',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 120 and pv.voucher_status='active') as 'E120',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 90 and datediff(now(), pv.purchase_date) <= 120 and pv.voucher_status='active') as 'D91',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 90 and datediff(now(), pv.bill_date) <= 120 and pv.voucher_status='active') as 'E91',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 75 and datediff(now(), pv.purchase_date) <= 90 and pv.voucher_status='active') as 'D76',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 75 and datediff(now(), pv.bill_date) <= 90 and pv.voucher_status='active') as 'E76',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 60 and datediff(now(), pv.purchase_date) <= 75 and pv.voucher_status='active') as 'D61',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 60 and datediff(now(), pv.bill_date) <= 75 and pv.voucher_status='active') as 'E61',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 45 and datediff(now(), pv.purchase_date) <= 60 and pv.voucher_status='active') as 'D46',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 45 and datediff(now(), pv.bill_date) <= 60 and pv.voucher_status='active') as 'E46',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 30 and datediff(now(), pv.purchase_date) <= 45 and pv.voucher_status='active') as 'D31',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 30 and datediff(now(), pv.bill_date) <= 45 and pv.voucher_status='active') as 'E31',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) > 15 and datediff(now(), pv.purchase_date) <= 30 and pv.voucher_status='active') as 'D16',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) > 15 and datediff(now(), pv.bill_date) <= 30 and pv.voucher_status='active') as 'E16',
										(SELECT sum(pvi.net_amount) from test_purchase_voucher_item pvi left join test_purchase_voucher pv on pv.purchase_voucher_id=pvi.purchase_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.purchase_date) >= 0 and datediff(now(), pv.purchase_date) <= 15 and pv.voucher_status='active') as 'D0',
										(SELECT sum(pvi.net_amount) from test_expense_voucher_item pvi left join test_expense_voucher pv on pv.expense_voucher_id=pvi.expense_voucher_id where pv.supplier_id=p.supplier_id and datediff(now(), pv.bill_date) >= 0 and datediff(now(), pv.bill_date) <= 15 and pv.voucher_status='active') as 'E0'
										from (select c.supplier_id,c.supplier_name,c.opening_balance,
										(select sum(pvi.net_amount) from test_purchase_voucher pv,test_purchase_voucher_item pvi where pv.purchase_voucher_id=pvi.purchase_voucher_id and pv.supplier_id=c.supplier_id) as net,
										(select sum(evi.net_amount) from test_expense_voucher ev,test_expense_voucher_item evi where ev.expense_voucher_id=evi.expense_voucher_id and ev.supplier_id=c.supplier_id) as enet,
										(select sum(payment_amount) from test_make_payment where supplier_id=c.supplier_id) as payment
										from test_supplier c
									...
									<c:set value="0" var="day120above"/>
                 					<c:forEach items="${ccustomer.rows}" var="rs">
	                 					...
	                 					<!--<c:set value="0" var="D0payment"/>-->
	                 					<c:if test="${(rs.net-rs.total_tds-rs.payment)>0}">
                 						<tr>
                 	...
                 	<c:otherwise>
                 		<!-- Get customer details here -->
	               		<sql:query var="supplier" dataSource="jdbc/rmc">
	               			select * from test_supplier where supplier_id=?
	               				...
	               				<table class="table table-bordered table-hover table-condensed printMe" id="example-2">
	               					...
	               					<tr>
	               						<td colspan="6" style="width:50%;">
	               							<table class="table table-bordered table-condensed">
	                							...
	                							<sql:query var="opening" dataSource="jdbc/rmc">
	                								select opening_balance from test_supplier where supplier_id=?
	                								<sql:param value="${param.supplier_id}"/>
	                							</sql:query>
	                							...
	                							<sql:query var="invoice" dataSource="jdbc/rmc">
	                								...
	                								sum(pvi.net_amount) as net_amount from test_purchase_voucher p,test_purchase_voucher_item pvi
	                								...
	                								sum(pvi.net_amount) as net_amount from test_expense_voucher p,test_expense_voucher_item pvi
				                						...
				                						<c:choose>
				                							<c:when test="${in.type=='Pur'}">
				                								<sql:query var="pvi" dataSource="jdbc/rmc">
					                								select pvi.item_name,pvi.item_quantity,pvi.item_price,round(pvi.net_amount,2) as net_amount from test_purchase_voucher_item pvi
				                							...
				                							<c:otherwise>
				                								<sql:query var="pvi" dataSource="jdbc/rmc">
					                								select pvi.item_name,pvi.item_quantity,pvi.item_price,round(pvi.net_amount,2) as net_amount from test_expense_voucher_item pvi
	               						...
	               						<sql:query var="payment" dataSource="jdbc/rmc">
	               							select * from test_make_payment where supplier_id=?
	               							<sql:param value="${param.supplier_id}"/>
	               						</sql:query>
	               						...
	               						<sql:query var="sumnet" dataSource="jdbc/rmc">
	               							select sum(payment_amount) as paysum from test_make_payment where supplier_id=?
	               							<sql:param value="${param.supplier_id}"/>
	               						</sql:query>
    	...
    	$(document).ready(function(){
    		...
    		var getSupplierListByBusinessGroup=function(business_id){
    			if(business_id!=undefined && !isNaN(business_id)){
    				$.ajax({
        				type:'post',
        				url:'../SupplierControllerTest?action=getSupplierListByBusinessGroup&business_id='+business_id,
        				headers:{
        					Accept:"application/json;charset=utf-8",
        					"Content-Type":"application/json;charset=utf-8"
        				},
