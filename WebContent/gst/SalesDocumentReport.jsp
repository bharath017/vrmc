                        ...
                        <div class="page-title-box">
                            <div class="btn-group pull-right">
                                <ol class="breadcrumb hide-phone p-0 m-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">Sales Document Report</a></li>
                                </ol>
                            </div>
                            <h4 class="page-title">Sales Document Report</h4>
                        </div>
                    </div>
                </div>
                 <!-- end row -->
                    ...
                    <div class="col-md-12">
                       <form action="#" id="myform" method="post">
                                   ...
                                   <div class="form-group">
                                       ...
                                       <select class="select2 no-date cus no-veh cus-dat no-veh-dat no-mar" name="customer_id" id="customer_id" required="required">
                                           ...
                                           <option value="${customer.customer_id}" 
                                           			${(param.customer_i==customer.customer_id)?'selected':''}>${customer.customer_name}</option>
                                           </c:forEach>
                                       </select>
                                   </div>
                               </div>
                          ...
                          <div class="text-center m-t-20">
                               ...
                               <a class="btn btn-danger btn-lg" href="SalesDocumentReport.jsp">Clear Report</a>
                          </div>
                       </form>
                    </div>
                </div>
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							select c.customer_name,s.site_name,s.site_address,c.customer_phone,c.customer_email,c.customer_address,
               							...
               							from  test_sales_document_item si LEFT JOIN(test_sales_document i,test_customer c,test_site_detail s,test_purchase_order po,test_purchase_order_item poi,product p,plant pl)
               							ON si.id=i.id
							...
							<thead>
			               				...
			               				<tr style="background-color: #4b9663;color: white;">
											...
											<td class="d-none">HSN Code</td>
			               					...
			               					<c:catch var="ee">
			               						<sql:query var="customer" dataSource="jdbc/rmc">
				               						...
				               						from test_sales_document i LEFT JOIN(test_customer c,test_purchase_order_item poi,test_sales_document_item si)
				               				...
				               				<c:forEach var="customer" items="${customer.rows}">
				               					<c:catch var="er">
				               						<sql:query var="invv" dataSource="jdbc/rmc">
				               							...
				               							c.customer_gstin,c.customer_address,s.site_address,
														...
														from test_sales_document i LEFT JOIN(test_customer c,test_site_detail s,product p,test_purchase_order_item poi,plant pl,test_sales_document_item si)
														ON i.customer_id=c.customer_id 
														and i.id=si.id
														and i.site_id=s.site_id 
														and si.poi_id=poi.poi_id 
               									...
               									<c:forEach var="invv" items="${invv.rows}">
													...
													<tbody>
               												<tr>
								               					...
								               					<td><a href="PrintSalesDocument.jsp?id=${invv.id}" target="_blank">${invv.invoice_id}</a></td>
								               					<td class="d-none">${customer.customer_name}</td>
								               					<td class="d-none">${invv.customer_gstin}</td>
								               					<td>${invv.site_address}</td>
								               					<td class="d-none">${invv.site_address}</td>
								               					<td>${invv.product_name}</td>
																<td class="d-none">38245010</td>
								               					<td><fmt:formatNumber value="${invv.item_quantity}" maxFractionDigits="2" groupingUsed="false"/></td>
								               					<c:choose>
																	<c:when test="${invv.rate_include_tax=='yes'}">
																		<td><fmt:formatNumber value="${(invv.item_price/118)*100}" maxFractionDigits="2" groupingUsed="false"/></td>
																	</c:when>
																	<c:otherwise> 
																		<td><fmt:formatNumber value="${invv.item_price}" maxFractionDigits="2" groupingUsed="false"/></td>
																	 </c:otherwise>
																</c:choose> 
								               					<td>${invv.gross_price}</td>
								               					<td class="d-none">${invv.tax_price}</td>
								               					<td class="d-none"><fmt:formatNumber value="${invv.tax_price/2}" groupingUsed="false" maxFractionDigits="2"/></td>
								               					<td class="d-none"><fmt:formatNumber value="${invv.tax_price/2}" groupingUsed="false" maxFractionDigits="2"/></td>
								               					<td class="d-none">0</td>
								               					<td>${invv.net_price}</td>
								               					<td>${invv.plant_name}</td>
								               					<td class="d-none">${invv.customer_address}</td>
								               					<td class="d-none">${invv.mp_name}</td>
								               					<td class="d-none">${invv.pump}</td>
								               					<td class="d-none">${invv.invoice_time}</td>
								               					<td>${invv.vehicle_no}</td>
								               					<td><input type="checkbox" id="id"  class="id idcls" name="idd" value="${invv.id}"/></td>
								               				</tr>
														</tbody>
								               				<c:set value="${date_count+invv.item_quantity}" var="date_count"/>
								               				<c:set value="${date_net+invv.net_price}" var="date_net"></c:set>
               												<c:set value="${date_gross+invv.gross_price}" var="date_gross"></c:set>
               												
               												<!-- Calculate total details here -->
               												<c:set value="${total_quantity+invv.item_quantity}" var="total_quantity"/>
						               						<c:set value="${total_netamount+invv.net_price}" var="total_netamount"/> 
						               						<c:set value="${total_gross_amount+invv.gross_price}" var="total_gross_amount"/>
               											</c:when>
               											<c:otherwise>
               												<tr class="text-right">
			               										...
			               										<td class="text-left"><b><fmt:formatNumber value="${date_net}" maxFractionDigits="2" groupingUsed="false"/></b></td>
			               										<td></td>
			               										<td></td>
			               										<td></td>
			               									</tr>
               										...
               										<tbody>
								               				...
								               				<c:set value="${data_count+invv.item_quantity}" var="date_count"/>
								               				<c:set value="${date_net+invv.net_price}" var="date_net"></c:set>
               												<c:set value="${date_gross+invv.gross_price}" var="date_gross"></c:set>
               									...
               									<tr class="text-right">
               										...
               										<td><b><fmt:formatNumber value="${date_net}" maxFractionDigits="2" groupingUsed="false"/></b></td>
               										<td></td>
               										<td></td>
               									</tr>
				               				</c:forEach>
											...
											<tr class="text-right">
												...
												<td colspan="3"><b><fmt:formatNumber value="${total_quantity}" maxFractionDigits="2" groupingUsed="false"/></b></td>
											</tr>
											<tr class="text-right">
											...
											<tr class="text-right">
												...
												<td colspan="3"><b><fmt:formatNumber value="${total_netamount}" maxFractionDigits="2" groupingUsed="false"/></b></td>
											</tr>		               				               				
               						</table>
               					</c:when>
               					<c:otherwise>
								...
								<tbody>
			               					<tr>
				               					...
				               					<td><a href="PrintSalesDocument.jsp?id=${inv.id}" target="_blank">${inv.invoice_id}</a></td>
												...
												<td class="d-none">38245010</td>
				               					...
				               					<c:choose>
													<c:when test="${inv.rate_include_tax=='yes'}">
														<td ><fmt:formatNumber value="${(inv.item_price/118)*100}" maxFractionDigits="2" groupingUsed="false"/></td>
													</c:when>
													<c:otherwise>
														<td><fmt:formatNumber value="${inv.item_price}" maxFractionDigits="2" groupingUsed="false"/></td>
													</c:otherwise>
												</c:choose>
				               					<td>${inv.gross_price}</td>
				               					<td>${inv.tax_price}</td>
				               					<c:choose>
				               						<c:when test="${inv.gst_category=='GST'}">
				               						<td class="d-none"><fmt:formatNumber value="${inv.tax_price/2}" groupingUsed="false" maxFractionDigits="2"/></td>
				               						<td class="d-none"><fmt:formatNumber value="${inv.tax_price/2}" groupingUsed="false" maxFractionDigits="2"/></td>
				               						<td class="d-none">0</td>
				               						</c:when>
				               						<c:otherwise>
				               						<td class="d-none">0</td>
				               						<td class="d-none">0</td>
				               						<td class="d-none"><fmt:formatNumber value="${inv.tax_price}" groupingUsed="false" maxFractionDigits="2"/></td>
       ...
       <%@ include file="../footer.jsp" %>
	    ...
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        }else if(report_type=='vehicle'){
	        	var vehicle_no=$('#vehicle_no').val();
	            var product_id=$('#product_id').val();
	            var plant_id=$('#plant_id').val();
	            window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	var plant_id=$('#plant_id').val();
	        	var block_name=$('#block_name').val();
	        	window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="SalesDocumentReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&mp_id="+mp_id;
	        }
