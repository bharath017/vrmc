                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                       	<sql:query var="cus" dataSource="jdbc/rmc">
                       		...
                       		(select sum(net_amount) from test_invoice where customer_id=c.customer_id) as net_amount,
                       		(select sum(payment_amount) from test_customer_payment where customer_id=c.customer_id) as net_paid,
                       		(select sum(sdi.net_price) from test_sales_document sd,test_sales_document_item sdi where sd.id=sdi.id and sd.customer_id=c.customer_id) as net_sales
                       		...
                       		<tbody>
                       			...
                       			<c:forEach items="${cus.rows}" var="cus">
                       				...
                       				<sql:query var="payment" dataSource="jdbc/rmc">
										...
										from test_invoice i LEFT JOIN (test_site_detail s,product p,test_purchase_order_item poi)
										...
										from test_sales_document_item sdi LEFT JOIN (test_sales_document sd,test_site_detail s,test_purchase_order_item poi,product p)
										ON sdi.id=sd.id
										...
										(select DATE_FORMAT(payment_date,'%d/%m/%Y') as date,'' as site_name,'' as product_name,0 as net_quantity,0 as rate,0 as total_amount,payment_amount as paid_amount,'' as age,payment_mode as payment_type from test_customer_payment where customer_id=? order by payment_date asc)
                       				...
                       				<c:forEach items="${payment.rows}" var="payment">
										...
										<tr>
											...
											<td><fmt:formatNumber value="${(payment.total_amount>0)?payment.total_amount:''}" maxFractionDigits="2"/></td>
                       	...
                       	<sql:query var="cus" dataSource="jdbc/rmc">
							select q.* from (
								...
								if(t.net_sales is null,0,t.net_sales) as net_sales,if(t.net_paid is null,0,t.net_paid) as net_paid
									...
									(select sum(i.net_amount) from test_invoice i where i.customer_id=c.customer_id) as net_amount,
									(select sum(sdi.net_price) from test_sales_document sd,test_sales_document_item sdi where sd.id=sdi.id and sd.customer_id=c.customer_id) as net_sales,
									(select sum(payment_amount) from test_customer_payment p where p.customer_id=c.customer_id) as net_paid
									from test_customer c
									where c.mp_id=?
                       		...
                       		<tbody>
                       			...
                       			<c:forEach items="${cus.rows}" var="cus">
                       				...
                       				<sql:query var="payment" dataSource="jdbc/rmc">
                       					(select DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as date,s.site_address as site_name,p.product_name as product_name,sum(i.quantity) as net_quantity,
                       					i.rate,sum(net_amount) as total_amount,'' as paid_amount,DATEDIFF(CURRENT_DATE(), i.invoice_date) as age,'' as payment_type
										from test_invoice i LEFT JOIN (test_site_detail s,product p,test_purchase_order_item poi,test_purchase_order po,test_customer c)
										ON i.site_id=s.site_id 
										and i.poi_id=poi.poi_id
										...
										(select DATE_FORMAT(sd.invoice_date,'%d/%m/%Y') as date,s.site_address  as site_name,p.product_name,sum(sdi.item_quantity) as net_quantity,
										sdi.item_price as rate,sum(net_price) as total_amount,'' as paid_amount,DATEDIFF(CURRENT_DATE(), sd.invoice_date) as age,'' as payment_type
										from test_sales_document_item sdi LEFT JOIN (test_sales_document sd,test_site_detail s,test_purchase_order_item poi,product p,test_customer c)
										ON sdi.id=sd.id
										and sd.site_id=s.site_id
										and sdi.poi_id=poi.poi_id
										and poi.product_id=p.product_id
                       			...
                       			<c:forEach items="${cus.rows}" var="cus">
                       				...
                       				<sql:query var="payment" dataSource="jdbc/rmc">
										...
										(select DATE_FORMAT(payment_date,'%d/%m/%Y') as date,'' as site_name,'' as product_name,
										 ...
										 payment_mode as payment_type from test_customer_payment where customer_id=?  and payment_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y') order by payment_date asc)
                   ...
                   <c:otherwise>
                   	<div class="col-md-12">
                       	<sql:query var="cus" dataSource="jdbc/rmc">
                       		select p.* from (select t.customer_id,t.customer_name,if(t.net_amount is null,0,t.net_amount) as net_amount,if(t.net_sales is null,0,net_sales) as net_sales,
                       		if(t.net_paid is null,0,t.net_paid) as net_paid from (select c.customer_id,c.customer_name,
                       		(select sum(net_amount) from test_invoice where customer_id=c.customer_id and invoice_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y')) as net_amount,
                       		(select sum(payment_amount) from test_customer_payment where customer_id=c.customer_id and payment_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y')) as net_paid,
                       		(select sum(sdi.net_price) from test_sales_document sd,test_sales_document_item sdi where sd.id=sdi.id and sd.customer_id=c.customer_id and sd.invoice_date between STR_TO_DATE(?, '%d/%m/%Y') and STR_TO_DATE(?, '%d/%m/%Y')) as net_sales
                       	    from test_customer c
                       	    where c.business_id like if(?=0,'%%',?)) as t) as p where ((p.net_amount+p.net_sales)-p.net_paid)>0;
                       	    <sql:param value="${param.from_date}"/>
                       	    <sql:param value="${param.to_date}"/>
