...
<head>
        <meta charset="utf-8" />
        <title>Old Customer Payment Report</title>
    ...
    <body>
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="payment" dataSource="jdbc/rmc">
               							select p.*,DATE_FORMAT(p.payment_date,'%d/%m/%Y') as payment_date,c.customer_name
               							from test_customer_payment_report p LEFT JOIN (test_customer c)
               					...
               					<table class="table" id="example-2">
		               				...
		               				<c:forEach items="${customer.rows}" var="customer">
										...
										<sql:query var="payment" dataSource="jdbc/rmc">
	               							...
	               							from test_customer_payment_report p
	               							where p.customer_id=?
	               							and p.payment_amount>0
       ...
       <%@ include file="../footer.jsp" %>
		...
		<script type="text/javascript" src="js/Accounts/Payment/CustomerPaymentReport.js"></script>
	    ...
	    $("form").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="OldCustomerPaymentReport.jsp?action=generateReport&business_id="+business_id+"&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date;
	        }else if(report_type=='customer'){
	        	var customer_id=$('#customer_id').val();
	        	var site_id=$('#site_id').val();
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="OldCustomerPaymentReport.jsp?action=generateReport&business_id="+business_id+"&report_type="+report_type+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id;
	        }
	        	...
	        	var from_date=$('input[name="from_date"]').val();
	        	var to_date=$('input[name="to_date"]').val();
	        	window.location="OldCustomerPaymentReport.jsp?action=generateReport&business_id="+business_id+"&report_type="+report_type+"&customer_id="+customer_id+"&from_date="+from_date+"&to_date="+to_date;
	        }
	    });
    </script>
