       ...
       <%@ include file="../header.jsp" %>
                    ...
                    <div class="col-12">
                        <div class="card-box table-responsive">
                             ...
                             <c:set value="${(empty param.batch_no)?'':param.batch_no}" var="batch_no"/>
                                <c:catch var="e">
                               		<c:choose>
                               			<c:when test="${param.plant=='plant1'}">
                               				<sql:query var="batch" dataSource="jdbc/rmc">
		                               			select t.* from (select NO_OF_CYCLES_VAL0,date(timestamp) as dt,POURING_DET_VAL0,TRUCK_DETAIL_VAL0,MIX_DESIGN_VAL0,TRUNCATE(TOTAL_M3_VAL0,2) as TOTAL_M3_VAL0,CUST_DETAIL_VAL0 from indus1) as t
		                               		    where NO_OF_CYCLES_VAL0 like if(''=?,'%%',?)
		                               		    and dt like if(''=?,'%%',?)
		                               		    group by NO_OF_CYCLES_VAL0,dt,POURING_DET_VAL0,TRUCK_DETAIL_VAL0,MIX_DESIGN_VAL0,TOTAL_M3_VAL0,CUST_DETAIL_VAL0  ORDER BY NO_OF_CYCLES_VAL0 DESC
		                               		    limit 100;
                               			...
                               			<c:otherwise>
                               				<sql:query var="batch" dataSource="jdbc/rmc">
		                               			select t.* from (select NO_OF_CYCLES_22_VAL0,date(timestamp) as dt,POURING_DET_VAL0,TRUCK_DETAIL_VAL0,MIX_DESIGN_VAL0,TRUNCATE(TOTAL_M3_VAL0,2) as TOTAL_M3_VAL0,CUST_DETAIL_VAL0 from ved_log) as t
		                               		    where NO_OF_CYCLES_22_VAL0 like if(''=?,'%%',?)
		                               		    and dt like if(''=?,'%%',?)
		                               		    group by NO_OF_CYCLES_22_VAL0,dt,POURING_DET_VAL0,TRUCK_DETAIL_VAL0,MIX_DESIGN_VAL0,TOTAL_M3_VAL0,CUST_DETAIL_VAL0  ORDER BY NO_OF_CYCLES_22_VAL0 DESC
		                               		    limit 100;
		                               			<sql:param value="${batch_no}"/>
		                               			<sql:param value="${batch_no}"/>
		                                		<sql:param value="${date}"/>
                            ...
                            <table id="example" class="table table-striped table-bordered table-condensed" cellspacing="0" width="100%">
                                ...
                                <c:forEach var="batch" items="${batch.rows}">
									<tr>
										<c:choose>
											<c:when test="${param.plant=='plant1'}">
												<td class="text-center">${batch.NO_OF_CYCLES_VAL0}</td>
											</c:when>
											<c:otherwise>
												<td class="text-center">${batch.NO_OF_CYCLES_22_VAL0}</td>
											</c:otherwise>
										</c:choose>
										<td class="text-center">${batch.dt}</td>
										<td class="text-center">${batch.POURING_DET_VAL0}</td>
										<td class="text-center">${batch.CUST_DETAIL_VAL0}</td>
										<td class="text-center">${batch.MIX_DESIGN_VAL0}</td>
										<td class="text-center">${batch.TOTAL_M3_VAL0}</td>
										<td class="text-center">${batch.TRUCK_DETAIL_VAL0}</td>
										<td class="text-center">
											 <div class="btn-group dropdown">
	                                            ...
	                                            <div class="dropdown-menu dropdown-menu-right">
	                                            	<c:choose>
	                                            		<c:when test="${param.plant=='plant1'}">
	                                            			<a class="dropdown-item" href="BatchSheetIndusClientPlant.jsp?id=${batch.NO_OF_CYCLES_VAL0}"><i class="fa fa-print mr-2 text-muted font-18 vertical-middle"></i>Print Batch Sheet</a>
	                                            		</c:when>
	                                            		<c:otherwise>
	                                            			<a class="dropdown-item" href="BatchSheetIndusClientPlant2.jsp?id=${batch.NO_OF_CYCLES_22_VAL0}"><i class="fa fa-print mr-2 text-muted font-18 vertical-middle"></i>Print Batch Sheet</a>
	                                            		</c:otherwise>
	                                            	</c:choose>
	                                            </div>
	                                        </div>
	    ...
	    $(document).ready(function(){
	    	$('#search').on('click',function(){
	    		...
	    		window.location='BatchingList.jsp?date='+invoice_date+'&batch_no='+batch_no;
	    	});
	    });
    </script>
    <script type="text/javascript">
