              	...
              	<c:choose>
              		<c:when test="${param.action=='update'}">
                		...
                		<sql:query var="cash" dataSource="jdbc/rmc">
                			select p.*,DATE_FORMAT(p.date,'%d/%m/%Y') as date,cat.category_name,cat.category_type,cat.category_id
                		    from test_petty_cash p LEFT JOIN (expense_category cat)
                		...
                		<c:forEach items="${cash.rows}" var="cash">
              			...
              			<div class="row">
		                    <div class="col-sm-12">
		                    	<form class="" id="myform" action="../PettyCashControllerTest?action=UpdatePettyCash" method="post">
			                        	...
			                        	<div class="col-sm-1"></div>
			                           	<div class="col-sm-6">
			                                ...
			                                <div class="form-group">
												...
												<input type="text" name="receiver" id="receiver" value="${rs.receiver}" class="form-control"/>
			                                </div>
			                                
			                                <div class="form-group">
			                                    <label>Expenditure Group <span class="text-danger">*</span></label>
			                                    <sql:query var="cattype" dataSource="jdbc/rmc">
			                                    	SELECT * FROM expense_category_type
			                                    </sql:query>
												<select class="select2" name="category_type"
														 id="cat_type" required="required">
														 <c:forEach items="${cattype.rows}" var="cattype">
														 	<option value="${cattype.category_type}"
														 			 ${(cattype.category_type==rs.category_type)?'selected':''}>
			                                ...
			                                <div class="form-group">
												...
												<select class="select2" name="category_id"
													...
													<c:forEach items="${category.rows}" var="category">
														<option value="${category.category_id}" ${(category.category_id==rs.category_id)?'selected':''}>${category.category_name}</option>
													</c:forEach>
		                                        </select>
			                                </div>
              		...
              		<c:otherwise>
              			<div class="row">
		                    <div class="col-sm-12">
		                    	<form class="" id="myform" action="../PettyCashControllerTest?action=InsertPettyCash" method="post">
			                        <div class="card-box row col-sm-12">
			                        	<div class="col-sm-1"></div>
			                           	<div class="col-sm-6">
			                           	   <div class="form-group">
			                                ...
			                                <div class="form-group">
			                                    <label>Expenditure Group <span class="text-danger">*</span></label>
												<select class="select2" name="category_type"
														 id="category_type" required="required">
