               			...
               			<table class="table table-bordered" id="example-2">
               				...
               				<c:forEach items="${market.rows}" var="market">
								...
								<sql:query var="report" dataSource="jdbc/rmc">
									select p.*, (select sum(payment_amount) from test_customer_payment where customer_id=p.customer_id) as totalpayment,
									(select sum(net_amount) from test_invoice where customer_id=p.customer_id) as totalamount
								    from  (select p.plant_name,sum(i.quantity) as tquantity,c.customer_name,c.customer_id
									from test_invoice i LEFT JOIN(test_customer c,test_purchase_order po,test_purchase_order_item poi,plant p)
               					...
               					<c:forEach var="report" items="${report.rows}">
		               				...
		               				<c:set value="${total_paid+report.totalamount}" var="total_amount"/>
