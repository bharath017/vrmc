$(document).ready(function(){
	...
	$('.picker').on('click',function(){
	 ...
	 var table =   $('#example').DataTable({
	        ...
	        lengthChange: true,
	        "ajax":{
	        	url:"../ExpenseVoucherControllerTest?action=getAllExpenseVoucherList",
	        	type:"POST",
	        	 data 	: function ( d ) {
		                  return $.extend( {}, d, {
	        ...
	        "columnDefs":[
	        	...
	        	"render":function(data,row){
		           ...
		           if((session.usertype=='sadmin' || session.usertype=='admin' || session.userype=='accounts' || session.usertype=='accountsStore') && data.voucher_status=='cancelled')
		            ...
		            //Delete   option
		 		   if((session.usertype=='sgst' || session.usertype=='gstaccount' || session.usertype=='gstbilling'  || session.usertype=='accounts') && data.voucher_status=='active')
	  ...
	  //Cancel purchase voucher here
	    	    ...
	    	    buttons: {
	    	        confirm:{
	    	            ...
	    	            action: function(){
	    	                $.ajax({
	    	                	type:'post',
	    	                	url:'../ExpenseVoucherControllerTest?action=CancelExpenseVoucher&expense_voucher_id='+expense_voucher_id,
	    	    ...
	    	    buttons: {
	    	        confirm:{
	    	            ...
	    	            action: function(){
	    	                $.ajax({
	    	                	type:'post',
	    	                	url:'../ExpenseVoucherControllerTest?action=deleteExpenseVoucher&expense_voucher_id='+expense_voucher_id,
	    	                	success:function(res){
	    	                		table.draw();
	    	                		$.toast({
	    ...
	    let getDashBoard=() => {
	    	$.ajax({
	    		type:'post',
	    		url:'../ExpenseVoucherControllerTest?action=getDashboard',
	    		headers:{
