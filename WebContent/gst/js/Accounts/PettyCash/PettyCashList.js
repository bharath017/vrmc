$(document).ready(function(){
	...
	var table =   $('#example').DataTable({
        ...
        "columnDefs":[
        	...
        	"render":function(data,row){
        		return (data.received_by=='' || data.received_by==undefined)?'N/A':data.received_by;
        	}},
        	...
        	"render":function(data,row){
        		return (data.category_name)?data.category_name:'N/A';
        	}},
        	...
        	"render":function(data,row){
                ...
                html+='<a href="javascript: void(0);" class="table-action-btn dropdown-toggle arrow-none btn btn-light btn-sm" data-toggle="dropdown" aria-expanded="false"><i class="mdi mdi-dots-horizontal"></i></a>'
                	html+='<div class="dropdown-menu dropdown-menu-right">'
                		if((session.usertype=='sgst' || session.usertype=='gst') && data.approve_status=='pending')
    ...
    $('#delete-btn').click(function(){
    	var cash_id=$('#cash_id-delete').val();
	    	$.ajax({
	    		type:'post',
	    		url:'../PettyCashControllerTest?action=DeletePettyCash&cash_id='+cash_id,
	    		headers:{
