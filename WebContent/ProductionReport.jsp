               		...
               		<div class="col-sm-12 printme table-responsive" id="printme">
               			<!-- Get all kind of query here -->
               				<sql:query var="report" dataSource="jdbc/rmc">
               					...
               					from invoice i LEFT JOIN(customer c,site_detail s,product p,purchase_order_item poi,purchase_order po,marketing_person m)
               			...
               			<table class="table table-bordered" id="example-2">
               				...
               				<c:forEach items="${report.rows}" var="report">
	               				...
	               				<c:set value="${total_amount+(report.rate*report.totalsum)}" var="total_amount"/>
               					<tr>
	               					<td style="width: 2%;">${count}</td>
	               					<td style="width: 15%;">${report.customer_name}</td>
	               					<td style="width:10%;">${report.customer_phone}</td>
	               					<td style="width: 20%;">${report.site_address}</td>
	               					<td style="width: 8%;">${report.product_name}</td>
	               					<td>${report.rate}</td>
	               					<td>${report.totalsum}</td>
	               					<td><fmt:formatNumber value="${report.rate*report.totalsum}" maxFractionDigits="2" groupingUsed="false"/></td>
	               					<td>${report.marketing_person}</td>
	               					<td>${report.pumps}</td>
               					...
               					<tr style="background-color: #70643b;color: white;">
									...
									<td><fmt:formatNumber value="${total_amount}" maxFractionDigits="2" groupingUsed="false"/></td>
									<td colspan="3"></td>
               					</tr>
               			</table>
               		</div>
