...
<head>
        <meta charset="utf-8" />
        <title>Old Invoice Report</title>
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,
               							(select gst_category from gst_percent where gst_id=po.gst_id) as gst_category,i.plant_id as plti_id,po.rate_include_tax,c.customer_name,s.site_name,s.site_address,pr.product_name,pr.product_id,p.plant_name,m.mp_name,c.customer_address,c.customer_gstin
               						    from invoice_report i LEFT JOIN (customer c,site_detail s,purchase_order_item poi,product pr,plant p,purchase_order po,marketing_person m)
               					...
               					<c:when test="${param.report_type=='customer'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,(select gst_category from gst_percent where gst_id=po.gst_id) as gst_category,i.plant_id as plti_id,po.rate_include_tax,c.customer_name,s.site_name,s.site_address,pr.product_name,pr.product_id,p.plant_name,m.mp_name,c.customer_address,c.customer_gstin
               						    from invoice_report i LEFT JOIN (customer c,site_detail s,purchase_order_item poi,product pr,plant p,purchase_order po,marketing_person m)
               							ON i.customer_id=c.customer_id
               							and i.site_id=s.site_id
               							and i.poi_id=poi.poi_id
               					...
               					<c:when test="${param.report_type=='vehicle'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               							select i.*,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') as invoice_date,po.rate_include_tax,
               							(select gst_category from gst_percent where gst_id=po.gst_id) as gst_category,i.plant_id as plti_id,c.customer_name,s.site_name,s.site_address,pr.product_name,pr.product_id,p.plant_name,m.mp_name,c.customer_address,c.customer_gstin
               						    from invoice_report i LEFT JOIN (customer c,site_detail s,purchase_order_item poi,product pr,plant p,purchase_order po,marketing_person m)
               							ON i.customer_id=c.customer_id
               							and i.site_id=s.site_id
               							and i.poi_id=poi.poi_id
							...
							<thead>
											...
											<td class="d-none">HSN Code</td>
			               					...
			               					<c:catch var="ee">
			               						<sql:query var="customer" dataSource="jdbc/rmc">
				               						...
				               						from invoice_report i LEFT JOIN(customer c,purchase_order_item poi)
				               						ON i.customer_id=c.customer_id 
				               						and i.poi_id=poi.poi_id
				               						where i.customer_id like if(0=?,'%%',?)
													and i.site_id like if(''=?,'%%',?)
				               				...
				               				<c:forEach var="customer" items="${customer.rows}">
				               					<c:catch var="er">
				               						<sql:query var="invv" dataSource="jdbc/rmc">
				               							select i.id,i.invoice_id,DATE_FORMAT(i.invoice_date,'%d/%m/%Y') AS invoice_date,i.pump,i.invoice_time,i.plant_id as plti_id,c.customer_name,c.customer_gstin,c.customer_address,s.site_address,
				               							i.quantity,i.net_amount,p.product_name,i.rate,i.tax_amount,i.gross_amount,pl.plant_name,i.vehicle_no
														from invoice_report i LEFT JOIN(customer c,site_detail s,product p,purchase_order_item poi,plant pl)
														ON i.customer_id=c.customer_id 
														and i.site_id=s.site_id 
														and i.poi_id=poi.poi_id 
				               					...
				               					<thead>
				               					<tr>
				               						<th colspan="11"><h4>${customer.customer_name}</h4></th>
				               					</tr>
				               					</thead>
               									...
               									<c:forEach var="invv" items="${invv.rows}">
													...
													<tbody>
               												<tr>
								               					...
								               					<td><a href="PrintInvoice.jsp?id=${invv.id}" target="_blank">${invv.invoice_id}</a></td>
														...
														<td class="d-none">38245010</td>
								               					...
								               					<c:choose>
																	<c:when test="${invv.rate_include_tax=='yes'}">
																		<td><fmt:formatNumber value="${(invv.rate/118)*100}" maxFractionDigits="2" groupingUsed="false"/></td>
																	</c:when>
																	<c:otherwise> 
																		<td><fmt:formatNumber value="${invv.rate}" maxFractionDigits="2" groupingUsed="false"/></td>
																	 </c:otherwise>
																</c:choose> 
								               					<td>${invv.gross_amount}</td>
								               					<td class="d-none">${invv.tax_amount}</td>
								...
								<tbody>
			               					<tr>
				               					...
				               					<td><a href="PrintInvoice.jsp?id=${inv.id}" target="_blank">${inv.invoice_id}</a></td>
												...
												<td class="d-none">38245010</td>
				               					...
				               					<c:choose>
													<c:when test="${inv.rate_include_tax=='yes'}">
														<td ><fmt:formatNumber value="${(inv.rate/118)*100}" maxFractionDigits="2" groupingUsed="false"/></td>
													</c:when>
													<c:otherwise>
														<td><fmt:formatNumber value="${inv.rate}" maxFractionDigits="2" groupingUsed="false"/></td>
													</c:otherwise>
												</c:choose>
				               					...
				               					<c:choose>
				               						...
				               						<td class="d-none">0</td>
				               						<td class="d-none">0</td>
				               						<td class="d-none"><fmt:formatNumber value="${inv.tax_amount}" groupingUsed="false" maxFractionDigits="2"/></td>
       ...
       <%@ include file="footer.jsp" %>
	    ...
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        }else if(report_type=='vehicle'){
	        	var vehicle_no=$('#vehicle_no').val();
	            var product_id=$('#product_id').val();
	            var plant_id=$('#plant_id').val();
	            window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id;
	        	...
	        	var plant_id=$('#plant_id').val();
	        	var block_name=$('#block_name').val();
	        	window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&customer_id="+customer_id+"&site_id="+site_id+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&vehicle_no="+vehicle_no+"&product_id="+product_id+"&plant_id="+plant_id+"&block_name="+block_name;
	        	...
	        	var product_id=$('#product_id').val();
	        	var plant_id=$('#plant_id').val();
	        	window.location="OldInvoiceReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&mp_id="+mp_id;
	        }
