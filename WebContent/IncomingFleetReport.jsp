...
<head>
        <meta charset="utf-8" />
        <title>Incoming Fleet Report</title>
    ...
    <body>
        ...
        <div class="wrapper">
            <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="btn-group pull-right">
                                <ol class="breadcrumb hide-phone p-0 m-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">Incoming Fleet Report</a></li>
                                </ol>
                            </div>
                            <h4 class="page-title">Incoming Fleet Report</h4>
                        </div>
                    </div>
                </div>
                 <!-- end row -->
                <div class="row">
                	<div class="col-md-12">
                		<h2 class="text-info text-center">Incoming Fleet Report</h2><hr><br><br>
                	</div>
                    <div class="col-md-12">
               	...
               	<div class="row" style="margin-top: 25px;">
               		...
               		<div class="col-sm-12 printme table-responsive" id="printme">
               				...
               				<c:choose>
               					<c:when test="${param.report_type=='date'}">
               						<sql:query var="inv" dataSource="jdbc/rmc">
               			               select fii.*,fi.item_name,f.* ,(select plant_name from plant where plant_id=f.plant_id) as plant_name
               			               from fleet_incoming_item fii LEFT JOIN(fleet_incoming f,fleet_item fi)
               			               ON fii.fleet_item_id=fi.fleet_item_id
               			               and fii.fleet_id=f.fleet_id
               			               where f.incoming_date between ? and ?
               			               and fii.fleet_item_id like if(''=?,'%%',?)
               			               and f.plant_id like if(''=?,'%%',?)
               			               order by fi_item_id desc 
               			...
               			<table class="table table-bordered" id="example-2">
               				...
               				<c:set value="0" var="net_sum"/>
               				<c:set value="0" var="gross_sum"/>
               				<c:set value="0" var="tax_sum"/>
							<c:set value="0" var="sum"/>               				
               				<c:forEach items="${inv.rows}" var="inv">
               				    <c:set value="${net_sum+inv.net_amount}" var="net_sum"/>
	               				<c:set value="${gross_sum+inv.gross_amount}" var="gross_sum"/>
	               				<c:set value="${tax_sum+inv.tax_amount}" var="tax_sum"/>
	               				<c:set value="${sum+1}" var="sum"/>
               					<tr>
	               					<td>${sum}</td>
	               					<td>${inv.fleet_supplier}</td>
	               					<td>${inv.item_name}</td>
	               					<td>${inv.incoming_date}</td>
	               					<td>${inv.incoming_time}</td>
	               					<td>${inv.quantity}</td>
	               					<td>${inv.rate}</td>
               					 ...
               					 <tr style="background-color: #70643b;color: white;">
               						...
               						<td colspan="2" class="text-right">Total  Gross Amount : </td>
               						<td colspan="1" class="text-left">
               							<fmt:formatNumber value="${gross_sum}" maxFractionDigits="2"/>
               						</td>
               						<td colspan="2" class="text-right">Total Tax Amount : </td>
               						<td colspan="1" class="text-left"><fmt:formatNumber value="${tax_sum}" maxFractionDigits="2"/></td>
               						<td colspan="1" class="text-right">Total Net Amount : </td>
               						<td colspan="1" class="text-left"><fmt:formatNumber value="${net_sum}" maxFractionDigits="2"/></td>
               					</tr> 
               			</table>
               		</div>
               	</div>
       ...
       <%@ include file="footer.jsp" %>
	    ...
	    $("#myform").submit(function(e){
	        ...
	        if(report_type=='date'){
	        	...
	        	window.location="IncomingFleetReport.jsp?action=generateReport&report_type="+report_type+"&from_date="+from_date+"&to_date="+to_date+"&fleet_item_id="+fleet_item_id+"&plant_id="+plant_id;
	        }
	    });
    </script>
