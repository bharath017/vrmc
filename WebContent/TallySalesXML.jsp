...
<c:set value="${(empty param.plant_id || param.plant_id=='null')?'':param.plant_id}" var="plant_id"/>
  ...
  <c:when test="${param.report_type=='date'}">
    <sql:query var="inv" dataSource="jdbc/rmc">
      ...
      (select sum(net_amount) from sales_document_item where id=i.id) as net_amount,
      (select sum(gross_amount) from sales_document_item where id=i.id) as gross_amount,
      (select sum(tax_amount) from sales_document_item where id=i.id) as tax_amount
      from sales_document i LEFT JOIN (customer c,site_detail s,plant p)
