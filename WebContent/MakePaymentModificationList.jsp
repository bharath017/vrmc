...
<head>
        <meta charset="utf-8" />
        <title>Make Payment Modification List</title>
    ...
    <body>
       ...
       <%@ include file="header.jsp" %>
                        ...
                        <div class="page-title-box">
                            <div class="btn-group pull-right">
                                <ol class="breadcrumb hide-phone p-0 m-0">
                                    ...
                                    <li class="breadcrumb-item active">Make Payment Modification List</li>
                                </ol>
                            </div>
                            <h4 class="page-title">Make Payment Modified List</h4>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-sm-3 col-xs-6">
                        <div class="form-group form-focus">
                            <label class="control-label">Voucher No</label>
                                <input class="form-control" placeholder="Enter Voucher No" id="voucher_no" type="text" name="voucher_no">
                        </div>
                    </div>
                            ...
                            <table id="example" class="table table-striped table-bordered table-condensed" cellspacing="0" width="100%">
                                <thead>
	                                <tr>
										...
										<th class="text-center">Old Amount</th>
                                 ...
                                 <!-- Get po details here -->
                                <c:set value="${(empty param.voucher_no)?'':param.voucher_no}" var="voucher_no"/>
                                <c:set value="${(empty param.payment_id)?'':param.payment_id}" var="payment_id"/>
                                <sql:query var="payment" dataSource="jdbc/rmc">
                                	select * from make_payment_modification where payment_id like if(''=?,'%%',?) and voucher_no like if(''=?,'%%',?) order by payment_modification_id desc
                                	<sql:param value="${payment_id}"/>
                                	<sql:param value="${payment_id}"/>
                                	<sql:param value="${voucher_no}"/>
                                	<sql:param value="${voucher_no}"/>
                                </sql:query>
                                <tbody>
                                <c:forEach items="${payment.rows}" var="payment">
                                   	<tr>
                                        ...
                                        <td class="text-center">${payment.modification_type}</td>
                                        <td class="text-center">${payment.modification_user}</td>
        ...
        <script type="text/javascript">
			function deleteMakePayment(payment_id){
	    ...
	    <script type="text/javascript">
		    $(document).ready(function(){
		    	$('#search').on('click',function(){
		    		...
		    		window.location='MakePaymentModificationList.jsp?voucher_no='+voucher_no+'&payment_id='+payment_id;
		    	});
		    });
	     </script>
