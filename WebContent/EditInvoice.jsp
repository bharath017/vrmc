...
<head>
    ...
    <body>
        ...
        <div class="wrapper">
            <div class="container-fluid">
               ...
               <sql:query var="invoice" dataSource="jdbc/rmc">
               		select i.*,i.plant_id as pltt_id,po.*,c.customer_name,c.customer_address,s.site_name,s.site_address,p.product_name,g.gst_percent as percent,g.gst_category as category
               	    from invoice i LEFT JOIN(customer c,site_detail s,purchase_order_item poi,purchase_order po,product p,gst_percent g)
              ...
              <c:forEach items="${invoice.rows}" var="invoice">
              	...
              	<div class="row">
                    <div class="col-sm-12">
                    	<form class="" id="myform" action="InvoiceController?action=UpdateInvoice" method="post">
	                        <div class="card-box row col-sm-12">
	                           	<div class="col-sm-6">
	                           		...
	                           		<div class="form-group">
										...
										<select id="plant_id"  name="plant_id" required="required"   class="select2"  data-placeholder="Choose Plant">
											...
											<option value="${plant.plant_id}" ${(plant.plant_id==rs.pltt_id)?'selected':''}>${plant.plant_name}</option>
											</c:forEach>
										</select>
	                                </div>
	                                ...
	                                <sql:query var="poi" dataSource="jdbc/rmc">
	                                    ...
	                                    from purchase_order_item  p,purchase_order po,product pro
	                                ...
	                                <div class="form-group">
	                                    ...
	                                    <div>
	                                       <input type="text" class="form-control" 
	                                         pattern="[^'&quot;:]*$" value="${rs.quantity}"  required="required"   id="quantity" name="quantity"/>
	                                    </div>
	                                </div>
	                                <div class="form-group">
	                            ...
	                            <div class="col-sm-6">
	                              <div class="form-group">
	                                ...
	                                <!-- Get all the block  -->
	                                <sql:query var="block" dataSource="jdbc/rmc">
	                                	select * from customer_site_block where site_id=?
	                                	<sql:param value="${rs.site_id}"/>
	                                </sql:query>
	                                <div class="form-group">
	                                    <label>Block(Optional)<span class="text-danger"> : </span></label>
										<select id="project_block"  name="project_block"  class="select2"  data-placeholder="Choose Site">
											<option value="">Please Select</option>
											<c:forEach items="${block.rows}" var="block">
												<option value="${block.block_name}" ${(block.block_name==rs.project_block)?'selected':''}>${block.block_name}</option>
											</c:forEach>
										</select>
	                                </div>
	                             </div>
	                             <div class="col-sm-12">
								  ...
								  <!-- write all hidden data here -->
								    <input type="hidden" name="total_nt_amt" id="total_nt_amt"/>
								  	<input type="hidden" name="tcs_required" id="tcs_required" value="0"/>
		                          	...
		                          	<input type="hidden" name="tcs_price" id="tcs_price" value="${rs.tcs_amount}"/>
	                             ...
	                             <div class="col-sm-12 table-responsive" id="clicked">
                                	<table class="table table-bordered" id="Table1">
                                		...
                                		<tr>
                                			<td class="change" style="width: 16.66%">TCS Amount : </td>
                                			<td class="nochange" style="width: 16.66%" id="tcs_view" >${rs.tcs_amount}</td>
                                			<td class="change" style="width: 16.66%">Balanced Quantity : </td>
                                			<td class="nochange" style="width: 16.66%" id="balanced_quantity"></td>
                                			<td class="change">IN WORDS</td>
                                			<td class="nochange" id="in-word"></td>
                                		</tr>
                                	</table>
                                </div>
       ...
       <!-- jQuery  -->
		...
		<script type="text/javascript" src="js/edit_invoice.js"></script>
